...
public class InfoAuditMenuCheckerImpl implements MenuTreeCheck {

	/** 信息权限 */
	public final static String INFO_CONFIG_GRANT = "info_config_grant";
	
	/** 信息审核 */
	public final static String INFO_CONFIG_GRANT_AUDIT = "info_config_grant_audit";
	...
	public boolean check() {
		...
		if(hasInfoPlugin) {
			try {
				return hasInfoGrant(CurrentUser.get().getAccountId(), CurrentUser.get().getId(), INFO_CONFIG_GRANT_AUDIT);
			} catch(Exception e) {
				e.printStackTrace();
				return false;
			}
