...
public class InfoReportMenuCheckerImpl implements MenuTreeCheck {

	/** 信息权限 */
	public final static String INFO_CONFIG_GRANT = "info_config_grant";
	
	/** 信息发布 */
	public final static String INFO_CONFIG_GRANT_REPORT = "info_config_grant_report";
	...
	public boolean check() {
		...
		if(hasInfoPlugin) {			
			try {
				return hasInfoGrant(CurrentUser.get().getAccountId(), CurrentUser.get().getId(), INFO_CONFIG_GRANT_REPORT);
			} catch(Exception e) {
				e.printStackTrace();
				return false;
			}
