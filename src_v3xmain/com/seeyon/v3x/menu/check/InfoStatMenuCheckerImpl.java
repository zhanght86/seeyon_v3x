...
public class InfoStatMenuCheckerImpl implements MenuTreeCheck {

	/** 信息权限 */
	public final static String INFO_CONFIG_GRANT = "info_config_grant";
	
	/** 信息统计 */
	public final static String INFO_CONFIG_GRANT_STAT = "info_config_grant_stat";
	...
	public boolean check() {
		...
		if(hasInfoPlugin) {
			try {
				return hasInfoGrant(CurrentUser.get().getAccountId(), CurrentUser.get().getId(), INFO_CONFIG_GRANT_STAT);
			} catch(Exception e) {
				e.printStackTrace();
				return false;
			}
