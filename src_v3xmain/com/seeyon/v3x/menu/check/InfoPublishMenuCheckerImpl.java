...
public class InfoPublishMenuCheckerImpl implements MenuTreeCheck {

	/** 信息权限 */
	public final static String INFO_CONFIG_GRANT = "info_config_grant";
	
	/** 信息发布 */
	public final static String INFO_CONFIG_GRANT_PUBLISH = "info_config_grant_publish";
	...
	public boolean check() {
		...
		if(hasInfoPlugin) {			
			try {
				return hasInfoGrant(CurrentUser.get().getAccountId(), CurrentUser.get().getId(), INFO_CONFIG_GRANT_PUBLISH);
			} catch(Exception e) {
				e.printStackTrace();
				return false;
			}
