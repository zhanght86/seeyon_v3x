...
public class InfoMagazineManagerMenuCheckerImpl implements MenuTreeCheck {

	/** 信息权限 */
	public final static String INFO_CONFIG_GRANT = "info_config_grant";
	
	/** 期刊管理 */
	public final static String INFO_CONFIG_GRANT_MAGAZINE_MANAGER = "info_config_grant_magazine_manager";
	...
	public boolean check() {
		...
		if(hasInfoPlugin) {
			try {
				return hasInfoGrant(CurrentUser.get().getAccountId(), CurrentUser.get().getId(), INFO_CONFIG_GRANT_MAGAZINE_MANAGER);
			} catch(Exception e) {
				e.printStackTrace();
				return false;
			}
