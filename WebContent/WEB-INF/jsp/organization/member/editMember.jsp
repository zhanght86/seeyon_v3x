...
<script type="text/javascript">
	function editvalidateTeamName(){
		...
		if ( valideLogin != memberNameValue){
	...
	function setLoginName(){
	    var read = '${readOnly}';
	    if(read != 'true'){
	        if(confirm(_("organizationLang.orgainzation_member_change_loginName"))){
	            document.getElementById("member.loginName").readOnly = false;
	            <c:if test="${editstate==null||!isModify}">
	            document.getElementById("member.password").value="";
	            document.getElementById("member.password1").value="";
	            </c:if>
	            document.getElementById("member.loginName").focus();
	        }else{
	            document.getElementById("member.loginName").readOnly = true;
	            document.getElementById("member.loginName").value = "${member.v3xOrgMember.loginName}";
	            document.getElementById("member.password").value = "${member.v3xOrgMember.password}";
	            document.getElementById("member.password1").value = "${member.v3xOrgMember.password}";
	            document.getElementById("member.loginName").blur();
	        }
	    }	    
	}
	
	function checkMember(){
		var orgPostId = document.getElementById("orgPostId").value;
	    var orgLevelId = document.getElementById("orgLevelId").value;
	    if(orgPostId == '-1'){
	        alert(_("organizationLang.orgainzation_org_not",'<fmt:message key="org.member_form.primaryPost.label" />','<fmt:message key="org.member.noPost"/>'));
	        return false;
	    }
	    if(orgLevelId == '-1'){
	    	alert(_("organizationLang.orgainzation_org_not",'<fmt:message key="org.member_form.levelName.label" />','<fmt:message key="org.member.noPost"/>'));
	        return false;
	    }
	    return true;	        
	}
	...
	function checkType() {
		...
		var disabledMemberTypeObj = document.getElementById("disabledMemberType");
		var disabledMemberTypeLabelObj = document.getElementById("disabledMemberTypeLabel");
		if (disabledMemberTypeObj && typeObj.value == disabledMemberTypeObj.value) {
			alert(_("organizationLang.orgainzation_member_type_disabling", disabledMemberTypeLabelObj.value));
			return false;
		}
		return true;
	}
...
<form id="memberForm" method="post" target="editMemberFrame" action="${organizationURL}?method=updateMember&deptAdmin=${param.deptAdmin}" onsubmit="var v = (submitOrgForm(checkForm(this) && checkSecret() &&tirmElementById('member.loginName') && checkPassword() && editvalidateTeamName() && checkMember() && checkType()${checkIdCard},this));if(v){enabledStateSelect()};return v;">
	...
	<input type="hidden" id="valideLogin" value="${member.v3xOrgMember.loginName}">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" align="center" class="">
	...
	<tr>
		<td height="42" align="center" class="bg-advance-bottom">
			...
			<input id="submintCancel" type="button" onclick="showDetail('${member.v3xOrgMember.id}','Member',parent.detailFrame)" value="<fmt:message key='common.button.cancel.label' bundle="${v3xCommonI18N}" />" class="button-default-2">
		</td>
	</tr>
	</c:if>
</table>
