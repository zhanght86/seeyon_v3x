 ...
 * 自定义团队/单位/集团空间图片新闻栏目  
 */
public class CustomImageNewsSection extends BaseSection {

	private static final Log log = LogFactory.getLog(CustomImageNewsSection.class);
	...
	@Override
	public String getId() {
		return "customImageNewsSection";
	}
	
	@Override
	public String getBaseName() {
		return "customImageNews";
	}

	@Override
	protected String getName(Map<String, String> preference) {
		return SectionUtils.getSectionName("customImageNews", preference);
	}
	...
	protected BaseSectionTemplete projection(Map<String, String> preference) {
		...
		try {
			...
			newsDatas = newsDataManager.findByReadUser4ImageNews(user.getId(), NumberUtils.toLong(spaceId), user.isInternal(), Constants.ImageNews, spaceType);
		} catch (Exception e) {
			log.error("", e);
		}
		...
		t.addBottomButton(BaseSectionTemplete.BOTTOM_BUTTON_LABEL_MORE, "/newsData.do?method=imageNewsMore&imageOrFocus=0&spaceType=" + spaceType + "&spaceId=" + spaceId);
		return t;
	}
