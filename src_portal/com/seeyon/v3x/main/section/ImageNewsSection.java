 ...
 * 单位图片新闻栏目
 */
public class ImageNewsSection extends BaseSection {

	private static final Log log = LogFactory.getLog(ImageNewsSection.class);
	...
	protected String getName(Map<String, String> preference) {
		return SectionUtils.getSectionName("imageNews", preference);
	}
	...
	protected BaseSectionTemplete projection(Map<String, String> preference) {
		...
		try {
			if ("designated".equals(panel)) {
				...
				newsDatas = newsDataManager.findByReadUser4ImageNews(user, Constants.ImageNews, NewsTypeSpaceType.corporation.ordinal(), typeList);
			} else {
				newsDatas = newsDataManager.findByReadUser4ImageNews(user, Constants.ImageNews, NewsTypeSpaceType.corporation.ordinal(), null);
			}
		} catch (Exception e) {
			log.error("", e);
		}
		...
		String moreLink = "";
		moreLink = "/newsData.do?method=imageNewsMore&imageOrFocus=0&spaceType=1" + "&fragmentId=" + preference.get(PropertyName.entityId.name()) + "&ordinal=" + preference.get(PropertyName.ordinal.name());
		if (panelValue != null) {
