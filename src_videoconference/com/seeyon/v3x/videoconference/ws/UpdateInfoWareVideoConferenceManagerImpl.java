...
public class UpdateInfoWareVideoConferenceManagerImpl implements UpdateVideoConferenceManager {
	//记录日志
	private static final Log log = LogFactory.getLog(UpdateInfoWareVideoConferenceManagerImpl.class);
	 ...
	 * @return String类型返回值
	 */
	public String updateVideoConference(Map videoParamMap) {
        String result = "";
		...
		if(videoParamMap!=null){
			try {
				...
				sb.append("<action>" + "updateFixedMeeting" + "</action>");
				...
				log.info("当前用户:("+meetingUser.getName()+") 用户loginname:("+meetingUser.getLoginName()+") 更新视频会议TO红杉树XML："+sb.toString());
				return SendXMLToRedFir.send((String)videoParamMap.get("webBaseUrl")+"/integration/xml" , sb.toString());
			} catch (Exception e) {
				log.error("更新视频会议出现错误："+e);
				return result;
			}
		}
