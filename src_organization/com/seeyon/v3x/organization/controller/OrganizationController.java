...
public class OrganizationController extends BaseController {
	...
	public ModelAndView showTopMenu(HttpServletRequest request,
	        HttpServletResponse response) throws Exception {
        ModelAndView modelAndView = new ModelAndView("organization/account/tree/topMenu");
	...
	public ModelAndView showRightMain(HttpServletRequest request,
	        HttpServletResponse response) throws Exception {
        ModelAndView modelAndView = new ModelAndView("organization/account/tree/rightMain");
        ...
        if("true".equals(request.getParameter("isRefreshTop"))){
            ...
            AccountSymbol accountSymbol = MainDataLoader.getInstance().getAccountSymbol(groupAccount.getId());
	...
	public ModelAndView showLeftTree(HttpServletRequest request,
	        HttpServletResponse response) throws Exception {
	    ModelAndView modelAndView = new ModelAndView("organization/account/tree/leftTree");
        String parentId = request.getParameter("parentId");
        if(Strings.isNotBlank(parentId)){
        	modelAndView.addObject("parentId", parentId);
        }
    ...
    public ModelAndView createAccountOfTree(HttpServletRequest request,
            HttpServletResponse response) throws Exception {
        ModelAndView result = new ModelAndView("organization/account/tree/editAccount");
        //获得当前排序号的最大值
        ...
        String enableMsgOrgConfig = this.systemConfig.get(IConfigPublicKey.MUCHORG_ENABLE);
    ...
    public ModelAndView editAccountOfTree(HttpServletRequest request,
            HttpServletResponse response) throws Exception {
        ModelAndView modelAndView = new ModelAndView("organization/account/tree/editAccount");
        String accountIdStr = request.getParameter("id");
        if(Strings.isNotBlank(accountIdStr)){            
            V3xOrgAccount account = orgManagerDirect.getAccountById(Long.parseLong(accountIdStr));
            orgManagerDirect.loadEntityProperty(account);
            V3xOrgMember member = orgManagerDirect.getMemberByLoginName(account.getAdminName());
            WebV3xOrgAccount webaccount = new WebV3xOrgAccount();
            webaccount.setV3xOrgAccount(account);
            ...
            List<V3xOrgAccount> accountlist = orgManagerDirect.getAllAccounts();
            //过滤掉自己和自己的子节点
            accountlist.removeAll(orgManagerDirect.getChildAccount(account.getId(), accountlist));
            ...
            if(isGovVersion){
            	webaccount.setAdminiLevel(account.getProperty("adminiLevel")); //政务版新增扩展属性  行政级别
            }
           ...
           if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
            {
                try
                {
                    V3xLdapRdn value=event.findLdapSet(Long.parseLong(accountIdStr));
    ...
    public ModelAndView updateAccountOfTree(HttpServletRequest request,
        ...
        if("create".equals(operation)){
            ...
            if(isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgAccount.class.getSimpleName(), "sortId",account.getSortId())){
                orgManagerDirect.insertRepeatSortNum(V3xOrgAccount.class.getSimpleName(), V3xOrgEntity.VIRTUAL_ACCOUNT_ID, account.getSortId());
            }
            //添加单位文档
//          docLibManager.addSysDocLibs(account.getId());       
            // 增加扩展属性
            account.setProperty("accountCategory",request.getParameter("accountCategory"));
            account.setProperty("accountLevel", request.getParameter("accountLevel"));
            account.setProperty("accountNature", request.getParameter("accountNature"));
            ...
            if(isGovVersion){
               account.setProperty("adminiLevel", request.getParameter("adminiLevel"));
            }
    		...
    		WorkTimeCurrency workTimeCurrency = workTimeSetManager.findComnWorkTimeSet(year, month, account.getId(), false);
            String[] beginTime = workTimeCurrency.getAmWorkTimeBeginTime().split(":");
            String[] endTime = workTimeCurrency.getPmWorkTimeEndTime().split(":");
    		workTimeSetManager.saveWorkTime(beginTime, endTime, account.getId());
			...
			if(!Strings.isBlank(aCategory)){
				...
				Metadata metadata = metadataManager.getMetadata(MetadataNameEnum.org_property_account_category);
				if(null!=metadata){
           ...
           /* V3xOrgMember member = new V3xOrgMember();            
            ...
            //记录应用日志
			appLogManager.insertLog(user,  AppLogAction.Organization_NewAccount,  user.getName(),  account.getName());      
			//是否复制集团职务级别
...
//            this.generateInitData(account.getId());            
            if(request.getParameter("cont") != null){
                return redirectModelAndView("/organization.do?method=createAccountOfTree&cont=true&reloadTree=true&parentId="+account.getSuperior()
                		+"&accountCategory="+account.getProperty("accountCategory")
                		+"&chiefLeader="+account.getProperty("chiefLeader")
                		+"&address="+account.getProperty("address")
                		+"&zipCode="+account.getProperty("zipCode")
                		+"&telephone="+account.getProperty("telephone")
                		+"&fax="+account.getProperty("fax")
                		+"&ipAddress="+account.getProperty("ipAddress")
                		+"&accountMail="+account.getProperty("accountMail")
                		+"&adminiLevel="+account.getProperty("adminiLevel")    //政务版新增行政级别属性
                ...
                out.println("<script>");
                out.println("alert('"+Constant.getString4CurrentUser("organization.ok")+"');");
                out.println("</script>"); 
                return redirectModelAndView("/organization.do?method=showRightMain&reloadTree=true&parentId="+account.getSuperior()+"&accountId="+account.getId());
            }
        }
        //修改单位的提交
        else{
            PrintWriter out = response.getWriter();
            boolean isGroupAccount = "true".equals(request.getParameter("isGroupAccount"));
            try {
                ...
                //2011-12-07lilong由于V3xOrgAccount不支持克隆，为增加监听添加此变量，后updateAccount(old, new)
                V3xOrgAccount oldAccount = new V3xOrgAccount();
                BeanUtils.copyProperties(oldAccount, account);
                // 已删除单位的校验
                if (account == null || (account.getIsDeleted() != null && account.getIsDeleted().booleanValue())) {
                	out.println("<script>");
					out.println("alert('"+ Constant.getString4CurrentUser("organization.update.no.account")+"');");
					out.println("</script>");
					out.flush();
    			...
    			if(orgEnabled&&!model.getEnabled()){
    				...
    				if(!isAccountCanBeDisenable(id)){
    					...
    					out.println("<script>");
    					out.println("alert('"+ Constant.getString4CurrentUser("orgainzation.unable.account")+"');");
    					out.println("</script>");
    					out.flush();
                ...
                if(isGroupAccount){
                    ...
                    String isInsert = request.getParameter("isInsert");
                    if(!orgSortId.equals(sortId)&&isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgAccount.class.getSimpleName(), "sortId", sortId,account.getId())){
                        orgManagerDirect.insertRepeatSortNum(V3xOrgAccount.class.getSimpleName(), CurrentUser.get().getLoginAccount(), sortId);
                    }               
                }
                ...
                // 更新管理员
                V3xOrgMember member = orgManagerDirect.getMemberByLoginName(oldLoginName);
                String adminName = request.getParameter("adminName");  
                String adminPass = request.getParameter("adminPass");
               ...
               /*if (!Strings.isEmpty(adminPass)){
                ...
                if(!oldLoginName.equalsIgnoreCase(adminName)&& !isGroupAccount){
                	// 管理员登录名改变，强制下线
                	OnlineRecorder.moveToOffline(oldLoginName, LoginOfflineOperation.adminKickoff);
                }
                //记录密码修改日志
    			if(!V3xOrgEntity.DEFAULT_INTERNAL_PASSWORD.equals(adminPass)&&!Strings.isEmpty(adminPass)){
                ...
                if(!account.getName().equals(oldName) || !account.getSuperior().toString().equals(oldSuperiorId)){
                    isReloadTree = true;
                }
                ...
                if(isGroupAccount){
                    if(!account.getName().equals(oldName) || !account.getSecondName().equals(request.getParameter("oldSecondName"))){
                        isRefreshTop = true;
                    }
                }
                ...
                //记录应用日志
    			appLogManager.insertLog(user,  AppLogAction.Organization_UpdateAccount,  user.getName(),  account.getName());            
                ...
                out.println("</script>");
                out.flush();
                return redirectModelAndView("/organization.do?method=showRightMain&reloadTree="+isReloadTree + "&isRefreshTop=" + isRefreshTop);
                ...
                out.println("</script>");
                out.flush();
                return redirectModelAndView("/organization.do?method=accountTreeIndex", "parent");
            }
        }
    }
	private void ldap4AccountOU(V3xOrgAccount account,String ldapOu, long id) throws BusinessException {
		if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
		{
		    try
		    {
    ...
    public ModelAndView destroyAccountOfList(HttpServletRequest request, 
    	...
    	if(strIds!=null && !strIds.equals("")){
    		...
    		if(!StringUtils.isBlank(cantBeDeletedAccounts)){
                out.println("<script>");
                out.println("alert(parent.v3x.getMessage('organizationLang.organization_accout_delete_no', '"+cantBeDeletedAccounts+"'));");
                out.println("</script>");
                out.flush();    			
    		}else{
        		//记录日志
        		appLogManager.insertLogs(user, AppLogAction.Organization_DeleteAccount, applogs);
                out.println("<script>");
    ...
    */
    ...
    public ModelAndView destroyAccountOfTree(HttpServletRequest request, 
        ...
        if (Strings.isNotBlank(accountIdStr)) {
            ...
            //记录应用日志
            User user = CurrentUser.get();
			appLogManager.insertLog(user,  AppLogAction.Organization_DeleteAccount,  user.getName(),  account.getName());            
	...
	public ModelAndView listAccount(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/account/listAccount");		
		...
		if("search".equals(request.getParameter("search"))){
			...
			strbuf.append("select a from " + V3xOrgAccount.class.getName() + " a where a.isDeleted='0' ");
			...
			sublist = orgManagerDirect.getEntityList(V3xOrgAccount.class.getSimpleName(), "isDeleted", false, V3xOrgEntity.VIRTUAL_ACCOUNT_ID);
		}
...
//			orgManagerDirect.setDefaultAccount(account.getId());
		...
		DataRecord dataRecord = OrganizationHelper.exportAccount(resultlist, request, metadataManager, response, fileToExcelManager);
		OrganizationHelper.exportToExcel(request, response, fileToExcelManager, company_list+"-" + userName, dataRecord);
		return null;
	...
	public ModelAndView createAccount(HttpServletRequest request,
		...
		String isInsert = request.getParameter("isInsert");
        if(isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgAccount.class.getSimpleName(), "sortId",account.getSortId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgAccount.class.getSimpleName(), CurrentUser.get().getLoginAccount(), account.getSortId());
		}
		// 添加单位文档
//		docLibManager.addSysDocLibs(account.getId());		
		// 增加扩展属性
        ...
        boolean isGovVersion = (Boolean)SysFlag.is_gov_only.getFlag();
        if(isGovVersion){
		  account.setProperty("adminiLevel", request.getParameter("adminiLevel"));  //政务新增扩展属性 行政级别
        }
		orgManager.addAccount(account);
		eventListener.addAccount(account);
		String aCategory = request.getParameter("accountCategory");
...
//		this.generateInitData(account.getId());
		...
		return redirectModelAndView("/organization.do?method=organizationFrame&from=Account&addOrganiza=addAccount&cont="+continues);
...
//		hr 管理-组织信息设置-单位信息-屏蔽单位角色管理标志
		...
		V3xOrgAccount account = null;
		if(isGroupAdmin){
			account = orgManagerDirect.getAccountById(Long.parseLong(request.getParameter("id")));
			//如果是根单位
			if(account.getIsRoot()){
				return redirectModelAndView("/organization.do?method=editGroupAccount&isGroupAdmin=true&isDetail="+isDetail);
			}
		}else{
            ...
            if("true".equals(request.getParameter("isRefreshTop"))){
                ...
                if(accountSymbol != null){
                    result.addObject("isRefreshAccountName", !accountSymbol.isHiddenAccountName());
                    if(!accountSymbol.isHiddenGroupName()){
                    	result.addObject("groupShortName", orgManagerDirect.getRootAccount().getShortname());
                    }
                }
            }
        ...
        boolean isGovVersion = (Boolean)SysFlag.is_gov_only.getFlag();
        if(isGovVersion){
		  webaccount.setAdminiLevel(account.getProperty("adminiLevel"));	  //政务新增扩展字段  行政级别
        }
        ...
        if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
        {
            try
            {
                V3xLdapRdn value=event.findLdapSet(CurrentUser.get().getLoginAccount());
	...
	public ModelAndView editGroupAccount(HttpServletRequest request,
        ...
        if("true".equals(request.getParameter("isRefreshTop"))){
            AccountSymbol accountSymbol = MainDataLoader.getInstance().getAccountSymbol(account.getId());
            if(accountSymbol != null){
                result.addObject("isRefreshAccountName", !accountSymbol.isHiddenGroupName());
            }
        }
	...
	public ModelAndView updateAccount(HttpServletRequest request,
		...
		try {
			...
			if(!isGroupAdmin){
				...
				if(!account.getName().equals(request.getParameter("oldName")) || !account.getSecondName().equals(request.getParameter("oldSecondName"))){
                    isRefreshTop = true;
                }
			}
			//扩展属性
			...
			if(orgEnabled&&!account.getEnabled()){
				...
				if(!isAccountCanBeDisenable(id)){
					account.setEnabled(orgEnabled);
					out.println("<script>");
					out.println("alert('"+ Constant.getString("orgainzation.unable.account",local)+"');");
					out.println("</script>");
					out.flush();
...
//			orgManagerDirect.updateEntity(member);
			...
			if (isGroupAdmin) {
				...
				return redirectModelAndView("/organization.do?method=editAccount&isGroupAdmin=false&isDetail=readOnly&unitManager=" + unitManager + "&isRefreshTop="+isRefreshTop);
			}
		} catch (BusinessException e) {
			out.println("<script>");
			out.println("alert('" + e.getMessage() + "!');");
	...
	public ModelAndView updateGroupAccount(HttpServletRequest request,
		...
		try {
            ...
            if(isGovVersion){
			  account.setProperty("adminiLevel", request.getParameter("adminiLevel"));  //政务新增 扩展字段 行政级别
            }
			// 更新管理员
//			V3xOrgMember member = orgManagerDirect.getMemberByLoginName(oldLoginName);
			String adminName = request.getParameter("adminName");	
...
//			orgManagerDirect.updateEntity(member);
			...
			if(!V3xOrgEntity.DEFAULT_INTERNAL_PASSWORD.equals(adminPass)&&!Strings.isEmpty(adminPass)){
				...
				appLogManager.insertLog(user, AppLogAction.Organization_UpdateGroupAdminPassword, groupName);
			}
			
            //记录应用日志			
			appLogManager.insertLog(user,  AppLogAction.Organization_UpdateGroupAccount,  account.getName());            
            boolean isRefreshTop = false; //更改单位名称时需要更新TOP
            if(!isGroupAdmin){
                if(!account.getName().equals(request.getParameter("oldName")) || !account.getSecondName().equals(request.getParameter("oldSecondName"))){
                    isRefreshTop = true;
			...
			if(isGroupAdmin){
				...
				return redirectModelAndView("/organization.do?method=editGroupAccount&isGroupAdmin=false&isDetail=readOnly&isRefreshTop="+isRefreshTop);
			}
		} catch (BusinessException e) {
			return redirectModelAndView("/organization.do?method=listAccount");
		}
	...
	public ModelAndView changerstate(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/account/changerState");
		String id = request.getParameter("id");
		V3xOrgAccount account = orgManagerDirect.getAccountById(Long.parseLong(id));
	 ...
	 */
	@CheckRoleAccess(roleTypes={RoleType.GroupAdmin})
	public ModelAndView changerAccountState(HttpServletRequest request,HttpServletResponse response){
		String id = request.getParameter("id");
		V3xOrgAccount account = null;
		try {
...
//		return redirectModelAndView("/organization.do?method=organizationFrame&from=Account");
	...
	public ModelAndView listDept(HttpServletRequest request,
		...
		ModelAndView result = new ModelAndView("organization/department/listDept");
		...
		if (Strings.isNotBlank(condition) && Strings.isNotBlank(textfield)) {
			if ( !condition.equals("orgDepartmentId")){
				strbuf.append("select a from " + V3xOrgDepartment.class.getName() + " a where a.orgAccountId='"+accountId+"' and a.isDeleted='0' and a.isInternal = '1' ");
				...
				strbuf.append(" order by a.sortId asc");
				deptlist = searchManager.searchByHql(strbuf.toString(), param, true);
			}else if (condition.equals("orgDepartmentId")) {
				V3xOrgDepartment dept = (V3xOrgDepartment)orgManagerDirect.getEntityNoRelation(V3xOrgDepartment.class.getSimpleName(), "id", Long.parseLong(textfield), accountId);
				String path = dept.getPath() + V3xOrgEntity.DEP_PATH_DELIMITER;
				strbuf.append("from " + V3xOrgDepartment.class.getName() + " where substring(path,1," + path.length() + ") ='"+path+"' and isDeleted ='0' and orgAccountId='"+accountId+"' and isInternal = '1' ");
			...
			for(Object dept : deptlist)
			{
				...
				//设置部门的岗位属性
				List<V3xOrgRelationship> rels = this.orgManagerDirect.getRelationships("type", V3xOrgEntity.ORGREL_TYPE_DEP_POST, "sourceId", dep.getId());
				for(Iterator<V3xOrgRelationship> relIt  = rels.iterator(); relIt.hasNext(); )
				{
					dep.addDepPost(relIt.next().getObjectiveId());
				}
			
				//设置部门的角色属性
				rels = this.orgManagerDirect.getRelationships("type", V3xOrgEntity.ORGREL_TYPE_DEP_ROLE, "sourceId", dep.getId());
				for(Iterator<V3xOrgRelationship> relIt  = rels.iterator(); relIt.hasNext(); )
				{
			...
			try {
				deptlist = orgManagerDirect.getEntityList(V3xOrgDepartment.class.getSimpleName(), "isInternal", true, accountId, true);
			}
			catch (BusinessException e) {
				log.error("", e);
			}
		...
		for (int i = 0; i < deptlist.size(); i++) {
			V3xOrgDepartment dept = (com.seeyon.v3x.organization.domain.V3xOrgDepartment) deptlist.get(i);		
			List<V3xOrgRole> roleList = new ArrayList<V3xOrgRole>();
			Iterator roleId = dept.getRoles().iterator();
			while(roleId.hasNext()){			
				V3xOrgRole role = orgManagerDirect.getRoleById(Long.parseLong(String.valueOf(roleId.next())));
				roleList.add(role);
			}	
			//决定表头显示几个角色
			if(i == 0){
		...
		for (int i = 0; i < deptlist.size(); i++) {
			...
			V3xOrgDepartment parent = orgManagerDirect.getParentDepartment(longid);
			...
			for(V3xOrgRole role : roleList){
				...
				ml = orgManagerDirect.getMemberByRole(role.getBond(), dept.getId(), role.getId());
				if(ml!=null&&ml.size()!=0){
					...
					if(role.getName().equals(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN))
							webdept.setAdminNames(membersb.toString());	
					else if(role.getName().equals(V3xOrgEntity.ORGENT_META_KEY_DEPMANAGER))
	...
	public ModelAndView exportDept(HttpServletRequest request,
		...
		V3xOrgAccount account = orgManagerDirect.getAccountById(user
				.getLoginAccount());
		//--------------MODIFY BY XUT 6-28----------------
		List<V3xOrgDepartment> deptlist = orgManagerDirect.getAllDepartments(user.getLoginAccount(),false);
		List<WebV3xOrgDepartment> resultlist = new ArrayList<WebV3xOrgDepartment>();
		for (int i = 0; i < deptlist.size(); i++) {
		...
		DataRecord dataRecord = OrganizationHelper.exportDept(resultlist, request, metadataManager, response, 
				fileToExcelManager, orgManagerDirect, spaceManager, account);
		OrganizationHelper.exportToExcel(request, response, fileToExcelManager, dept_list+"-"
				+ user.getLoginName(), dataRecord);
		return null;
	}
	...
	public ModelAndView addDept(HttpServletRequest request,
		...
		Locale local = LocaleContext.getLocale(request);
		//获得最大排序号
		Integer maxSortNum = orgManagerDirect.getMaxSortNum(V3xOrgDepartment.class.getSimpleName(), CurrentUser.get().getLoginAccount());
		...
		if (null != parentID && !parentID.equals("")) {
			V3xOrgDepartment parentDept = orgManagerDirect.getDepartmentById(Long.parseLong(parentID));
			...
			if(parentDept!=null&&!parentDept.getIsInternal()){
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert('"+ Constant.getString("depatition.isinternal.parent",local)+"');");					
				out.println("</script>");
				out.flush();
				return null;				
				...
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert('"+ Constant.getString("depatition.isenabled.parent",local)+"');");					
				out.println("</script>");
				out.flush();
				return null;
			}
		...
		while(allIt.hasNext()){
			V3xOrgRole role = orgManagerDirect.getRoleById(Long.parseLong(String.valueOf(allIt.next())));
			if(role.getName().equals(EdocRoleHelper.departmentExchangeRoleName)&&!Functions.isEnableEdoc()){
				continue;
			}
			roleList.add(role);
		}
	...
	public ModelAndView createDept(HttpServletRequest request,
		...
		try {
			...
			Long _parent_Id = 0L;
			if (null != parentId && !parentId.equals("")) {
				parent = orgManagerDirect.getDepartmentById(Long.parseLong(parentId));
			} else {
			...
			//加入同级部门的校验
			List<V3xOrgDepartment> depts = orgManagerDirect.getChildDepartments(_parent_Id, true);
			...
			//部门排序号的重复处理
			String isInsert = request.getParameter("isInsert");
            if(isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgDepartment.class.getSimpleName(), "sortId", dept.getSortId())){
            	orgManagerDirect.insertRepeatSortNum(V3xOrgDepartment.class.getSimpleName(), accountid, dept.getSortId());
			}
			...
			String isCreateDeptSpace =  request.getParameter("isCreateDeptSpace");
			...
			//创建部门的空间
			/*if (null != isCreateDeptSpace && isCreateDeptSpace.equals("1")){
				spaceManager.createDepartmentSpace(addedDept.getId(), addedDept.getName(), accountid);			
				//增加部门公告
			    ((BaseBulletinManager)bulDataManager).getBulletinUtils().createBulTypeByDept(dept.getName()+"公告",dept.getId(),accountid);
				
			    //~~~~~~~~~~~~~~动态增加部门讨论~~Tanmf~~~~~~~~
				this.bbsBoardManager.createDepartmentBbsBoard(dept.getId(), accountid, dept.getName());
			}*/
			super.rendJavaScript(response, "parent.doEndDept('"+accountid+"', 'add', '" + dept.getId() + "', '" + _parent_Id + "', '" + Strings.escapeJavascript(dept.getName()) + "', "+dept.getSortId()+")");
			
			// 触发创建部门事件
			eventListener.addDepartment(dept);
			
			//记录日志
			appLogManager.insertLog(user, AppLogAction.Organization_NewDept, user.getName(),dept.getName());
	...
	public ModelAndView editDept(HttpServletRequest request,
		...
		if(click!=null){			
			...
			if(click.equals("root")){
				ModelAndView resultRoot = new ModelAndView("organization/department/deptInfo");
				return resultRoot;
			}
		}
		...
		// 获得部门岗位
		List<V3xOrgPost> listPost = orgManagerDirect.getDepartmentPost(dept.getId());
		...
		// 获取是否为内部人员
		List<V3xOrgDepartment> deptInternal = orgManagerDirect.getChildDepartments(_id, true);
		...
		//获取是否创建了部门空间
		result.addObject("isCreateDepartmentSpace", spaceManager.isCreateDepartmentSpace(dept.getId()));
		// 在修改的是后显示 部门管理信息
		result.addObject("showDeptManager", 1);
	...
	public ModelAndView editDeptPosts(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/department/deptPosts");
		String strid = request.getParameter("id");
		Long _id = Long.parseLong(strid);
		V3xOrgDepartment dept = orgManagerDirect.getDepartmentById(_id);
		WebV3xOrgDepartment webdept = new WebV3xOrgDepartment();
		...
		V3xOrgRole deptManager = orgManagerDirect.getRoleByName(V3xOrgEntity.ORGENT_META_KEY_DEPMANAGER);
		List<V3xOrgMember> managerList = null;
		if(deptManager!=null){
		    managerList = orgManagerDirect.getMemberByRole(V3xOrgEntity.ROLE_BOND_DEPARTMENT, _id, deptManager.getId());			
		}
		List<WebV3xOrgMember> webMemberList = new ArrayList<WebV3xOrgMember>();
	    List<Long> memberIdList = new ArrayList<Long>();
		Locale local = LocaleContext.getLocale(request);
		String resource = "com.seeyon.v3x.organization.resources.i18n.OrganizationResources";
		String cntStr = ResourceBundleUtil.getString(resource, local, "department.cntPost");

		//取得本部门下的直属人员
		List<V3xOrgEntity> ents = orgManagerDirect.getEntityListNoRelation(V3xOrgMember.class.getSimpleName(), "orgDepartmentId", dept.getId(), dept.getOrgAccountId(), false);
		//为了提升性能这里将单位下的所有岗位缓存
//		Map postMap = organization.getAllEntity(V3xOrgPost.class, dept.getOrgAccountId());
		...
		//取得本部门下的副岗人员
		List<V3xOrgRelationship> memberPosts = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_MEMBER_POST,"objectiveId",dept.getId());
		if(memberPosts!=null&&memberPosts.size()>0){
			for(V3xOrgRelationship rel:memberPosts){
				if(!memberIdList.contains(rel.getSourceId())){
					V3xOrgMember member = (V3xOrgMember)orgManagerDirect.getMemberById(rel.getSourceId());
					if(member!=null&&member.getEnabled()){
						WebV3xOrgMember webMember = new WebV3xOrgMember();
						webMember.setV3xOrgMember(member);
						webMember.setTypeName(cntStr+member.getName());
						if(rel.getBackupId()!=null){
							V3xOrgPost post = orgManagerDirect.getPostById(rel.getBackupId());
							if(post!=null){
		...
		//取得本部门下的兼职人员
		List<V3xOrgRelationship> cntPosts = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_CONCURRENT_POST,"objectiveId",dept.getId());
	...
	public ModelAndView updateDeptPosts(HttpServletRequest request,
		...
		V3xOrgDepartment dept = orgManagerDirect.getDepartmentById(new Long(deptId));
		...
		for(V3xOrgPost post : listPost){
			if(!postList.contains(post.getId())&&post.getId()!=-1){
				//检查部门岗位下是否有人
				List<V3xOrgMember> members = orgManagerDirect.getMembersByDeptPost(post.getId(), dept.getId(), dept.getOrgAccountId());
				if(members!=null && !members.isEmpty()){
					PrintWriter out = response.getWriter();
					out.println("<script>");
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_del_dept_post_mem','"+post.getName()+"'));");
					out.println("</script>");
					return null;
				}
			}
	...
	public ModelAndView updateDept(HttpServletRequest request,
		...
		// 如果部门信息不存在则提示用户
		if (dept == null || dept.getIsDeleted() || !dept.getOrgAccountId().equals(user.getLoginAccount())) {
			PrintWriter out = response.getWriter();
			out.println("<script>");
		...
		boolean nowEnable = request.getParameter("enabled").equals("1")? true:false;
		//如果部门被停用则停用子部门
		if(orgEnable&&(!nowEnable)){
			//List<V3xOrgMember> deptMembers = orgManagerDirect.getMembersByDepartment(dept.getId(), false, CurrentUser.get().getLoginAccount());
			...
			V3xOrgDepartment parent = orgManagerDirect.getParentDepartment(lDeptId);
			if(parent!=null){
				if(!parent.getEnabled()){
					PrintWriter out = response.getWriter();
					out.println("<script>");
					out.println("alert('"+ Constant.getString("depatition.unenable.parent",local)+"');");				
					out.println("</script>");				
					super.rendJavaScript(response, "parent.toEditMember()");
					return null;
				}
		...
		Integer sortId = Integer.valueOf(request.getParameter("sortId"));
		String isInsert = request.getParameter("isInsert");
        if(!orgSortId.equals(sortId)&&isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgDepartment.class.getSimpleName(), "sortId", sortId,dept.getId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgDepartment.class.getSimpleName(), CurrentUser.get().getLoginAccount(), sortId);
		}	
        bind(request, dept);
        ...
        if(parentId!=null&&StringUtils.isNotBlank(parentId)){
            ...
            V3xOrgDepartment tempdept = orgManagerDirect.getDepartmentById(Long.parseLong(parentId));
		...
		//更新父部门
		/*V3xOrgDepartment orgParent = orgManagerDirect.getParentDepartment(lDeptId);	
		if(orgParent==null){
		...
		//加入同级部门的校验
		List<V3xOrgDepartment> depts = orgManagerDirect.getChildDepartments(Long.parseLong(parentId), true);
		for(V3xOrgDepartment orgDept:depts){
			if(orgDept.getName().equals(dept.getName())&&!orgDept.equals(dept)){
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_brother_dept_name'));");
				out.println("</script>");
		...
		boolean isCreateDeptSpace = "true".equals(request.getParameter("isCreateDeptSpace"));
		...
		for(int loop = 0;;loop++){
			if(request.getParameter(loop+"roleid")!=null){
				...
				Long[] ids = new Long[StringUtils.isBlank(memberIds)?0:memberID.length];
                for (int i=0;i<ids.length;i++) {
                	V3xOrgMember roleMem = orgManagerDirect.getMemberById(Long.parseLong(memberID[i]));
		        ...
		        List<V3xOrgMember> mems = orgManagerDirect.getMemberByRole(V3xOrgEntity.ROLE_BOND_DEPARTMENT, lDeptId, roleId);
...
//		        Collections.sort(roleMembers,CompareSortEntity.getInstance());
		        if(!Arrays.equals(mems.toArray(), roleMembers.toArray())){
		        	orgManagerDirect.addRole2Member(V3xOrgEntity.ROLE_BOND_DEPARTMENT,lDeptId, roleId, ids);
		        	//如果是部门管理员更改，而且原管理员存在，则强制原部门管理员下线
					if(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN.equals(request.getParameter("roleName"+loop))&&mems.size()>0){
						OnlineRecorder.moveToOffline(mems.get(0).getLoginName(), LoginOfflineOperation.adminKickoff);
					}
					//记录日志	
					List<String[]> appLogs = new ArrayList<String[]>();
					for (V3xOrgMember mem : roleMembers) {
						...
						appLog[2] = ResourceBundleUtil.getString(resource, locale, "sys.role.rolename."+role.getName());
						appLogs.add(appLog);
					}
					appLogManager.insertLogs(user, AppLogAction.Organization_ChangeDepRole, appLogs);
...
//			spaceManager.createDepartmentSpace(dept.getId(), dept.getName(), accounrId);
			//增加部门讨论区
//			this.bbsBoardManager.createDepartmentBbsBoard(dept.getId(), accounrId, dept.getName());
		
			//增加部门公告
			((BaseBulletinManager)bulDataManager).getBulletinUtils().createBulTypeByDept(dept.getName()
					+ResourceBundleUtil.getString(Constants.BUL_RESOURCE_BASENAME, "bul.more"), dept.getId(), accounrId);
			
			//部门更新后需要同时更新部门讨论版块、部门公告的管理员信息
			this.bbsBoardManager.updateDeptBBSBoardManager(lDeptId, deptAdmins);
			((BaseBulletinManager)bulDataManager).getBulletinUtils().updateDeptBulTypeManagers(lDeptId, deptAdmins);
		}		
...
//             spaceManager.deleteDepartmentSpace(dept.getId());
		...
		//记录日志
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateDept, user.getName(),dept.getName());
		...
		if(islist!=null && !islist.equals("")){
			super.rendJavaScript(response, "parent.parent.listFrame.location.reload(true);" +
		...
		if (null != strids && !strids.equals("")) {
			...
			if (null != arrids && arrids.length > 0) {
				for (String id : arrids) {
					...
					V3xOrgDepartment dept1 = orgManagerDirect.getDepartmentById(longid);
					
					deptList.add(dept1);
					//检查部门下是否存在成员
					List<V3xOrgMember> members = orgManagerDirect.getMembersByDepartment(dept1.getId(), false, dept1.getOrgAccountId());
					...
					if(ListUtils.EMPTY_LIST.equals(members)||isAllMemberUnEnabled){
						//检查部门下是否存在组
						List<V3xOrgTeam> teams = orgManagerDirect.getDepartmentTeam(longid);
						List<V3xOrgDepartment> childDeps = orgManagerDirect.getChildDepartments(longid, false);
						...
						for(V3xOrgTeam team:teams){
							if(team.getEnabled()&&team.getType()!=V3xOrgEntity.TEAM_TYPE_PERSONAL&&team.getType()!=V3xOrgEntity.TEAM_TYPE_DISCUSS){
						...
						if(ListUtils.EMPTY_LIST.equals(teams)||isAllTeamUnEnabled){
				           ...
				           //删除部门空间的讨论 ---------lucx--------
							/*try {
								this.bbsBoardManager.deleteV3xBbsBoard(dept1.getId());
							} catch (org.springframework.dao.DataIntegrityViolationException me) {
								me.printStackTrace();
							}
							//删除部门空间的公告---
							bulTypeManager.delDept(dept1.getId());
							spaceManager.deleteDepartmentSpace(dept1.getId());*/
						}else{
							PrintWriter out = response.getWriter();
							out.println("<script>");
							out.println("alert('"+ Constant.getString("organization.delete.team",local)+"');");
							out.println("</script>");
							out.flush();
//							return super.refreshWorkspace();
//							去掉上面的整个工作区刷新，改成按链接刷新。hr布局用
							if(islist!=null && !islist.equals("")){
								return super.redirectModelAndView("/organization.do?method=showframe&style=list","parent.parent");
							}else{
								return super.redirectModelAndView("/organization.do?method=showframe&style=tree","parent");
							}
						}
					}else{
						// 提示用户部门下有有效成员，不能删除
						PrintWriter out = response.getWriter();
						out.println("<script>");
						out.println("alert('"+ Constant.getString("depatition.delete.member",local)+"');");
						out.println("</script>");
						out.flush();
//						return super.refreshWorkspace();
//						去掉上面的整个工作区刷新，改成按链接刷新。hr布局用
        ...
        //记录日志
		appLogManager.insertLogs(user, AppLogAction.Organization_DeleteDept, appLogs);
...
//		去掉上面的整个工作区刷新，改成按链接刷新。hr布局用
	...
	public ModelAndView showtree(HttpServletRequest request,
		...
		User user = CurrentUser.get();
		V3xOrgAccount account = orgManagerDirect.getAccountById(user.getLoginAccount());
		ModelAndView result = new ModelAndView("organization/department/lefttree");
		List<V3xOrgDepartment> deptlist = new ArrayList<V3xOrgDepartment>();
		List<V3xOrgEntity> list = orgManagerDirect.getEntityListNoRelation(V3xOrgDepartment.class.getSimpleName(), "isInternal", true, account.getId());
	...
	public ModelAndView showframe(HttpServletRequest request,
		...
		if(style.equals("tree")){
			ModelAndView result = new ModelAndView("organization/department/treeIndex");
			return result;
		}else{
			ModelAndView result = new ModelAndView("organization/department/listIndex");
			return result;
		}
	}
	...
	public ModelAndView showmenu(HttpServletRequest request,
		...
		String style = request.getParameter("style");		
		if(style.equals("tree")){
			ModelAndView result = new ModelAndView("organization/department/treeMenu");
			return result;
		}else{
			ModelAndView result = new ModelAndView("organization/department/listMenu");
			return result;
		}
	}
	...
	public ModelAndView listPost(HttpServletRequest request,
		...
		try{
			//根据查询条件查询岗位列表			
			if(condition!=null
					&&Strings.isNotBlank(condition)
					&&textfield!=null
					&&Strings.isNotBlank(textfield)){
				StringBuffer strbuf = new StringBuffer();
				Map<String, Object> param = new HashMap<String, Object>();
				strbuf.append("select a from " + V3xOrgPost.class.getName() + " a where a.orgAccountId='"+user.getLoginAccount()+"' and a.isDeleted='0'");
				if(condition.equals("typeId")&&Strings.isNotBlank(textfield)){
					...
					strbuf.append(" order by a.sortId asc");
					List<V3xOrgEntity> lst = searchManager.searchByHql(strbuf.toString(), param, true);
		...
		for(V3xOrgPost post:postlist){
			...
			List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_BENCHMARK_POST,"sourceId",post.getId());
		...
		if(Boolean.valueOf(SysFlag.org_showAddBmPost.getFlag().toString())
			...
			result.addObject("showAddBmPost", SysFlag.org_showAddBmPost.getFlag());
		}
		...
		if(postIds!=null&&!StringUtils.isBlank(postIds)){
			...
			if(postIdArray!=null&&postIdArray.length>0){
				...
				for(int i=0;i<postIdArray.length;i++){
					...
					ispd = orgManagerDirect.isPropertyDuplicated(V3xOrgPost.class.getSimpleName(), "name", post.getName());					
					if(ispd){
				...
				if(isHavepd){
					PrintWriter out = response.getWriter();
					out.println("<script>");
				    out.println("alert('"+name+"'+ parent.v3x.getMessage('organizationLang.organization_double_name_2'));");					
					out.println("</script>");
					out.flush();					
				}	
				...
				for(Long postId:bmPostIds){
					orgManagerDirect.addBenchMarkPostRel(postId, user.getLoginAccount());		
               ...
               //记录日志
				appLogManager.insertLogs(user, AppLogAction.Organization_NewPost, applogs);
	...
	public ModelAndView bandPost(HttpServletRequest request,
		...
		if(postIds!=null&&!StringUtils.isBlank(postIds)){
			...
			if(postIdArray!=null&&postIdArray.length>0){
				...
				List<V3xOrgPost> bmPosts = orgManagerDirect.getAllPosts(orgManagerDirect.getRootAccount().getId(), false);
				...
				if(isHavepd){
					...
					out.println("<script>");	
				    out.println("alert(parent.v3x.getMessage('organizationLang.organization_band_bmpost_fail','"+postStr+"'));");					
					out.println("</script>");
					out.flush();		
				}
			}
	...
	public ModelAndView addPost(HttpServletRequest request,
		...
		//获得岗位的最大排序号
		Integer maxSortNum = orgManagerDirect.getMaxSortNum(V3xOrgPost.class.getSimpleName(), CurrentUser.get().getLoginAccount());
	...
	public ModelAndView createPost(HttpServletRequest request,
		...
		try {
			...
			if(user.isGroupAdmin()){
				List<V3xOrgEntity> entList = orgManagerDirect.getEntityListNoRelation(V3xOrgPost.class.getSimpleName(), "name", post.getName(), user.getLoginAccount());
			...
			//岗位排序号的重复处理
			String isInsert = request.getParameter("isInsert");
            if(isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgPost.class.getSimpleName(), "sortId", post.getSortId())){
            	orgManagerDirect.insertRepeatSortNum(V3xOrgPost.class.getSimpleName(), CurrentUser.get().getLoginAccount(), post.getSortId());
			}
			orgManager.addPost(post);
			//更改枚举项引用
			if(null!=post){
				long itemValue = post.getTypeId();
				Metadata metadata = metadataManager.getMetadata(MetadataNameEnum.organization_post_types);
				if(null!=metadata){
					MetadataItem item = metadataManager.getMetadataItem(metadata.getName(), Long.valueOf(itemValue).toString());
					if(null!=item){
						metadataManager.refMetadataItem(metadata.getId(), item.getId(), Long.valueOf(itemValue).intValue());
					}
				}
			}
			super.rendJavaScript(response, "parent.doEndPost('"+CurrentUser.get().getLoginAccount()+"')");
			
			// 触发创建岗位事件
			eventListener.addPost(post);

            //记录日志
			appLogManager.insertLog(user, AppLogAction.Organization_NewPost, user.getName(),post.getName());
	...
	public ModelAndView updatePost(HttpServletRequest request,
		...
		Integer sortId = Integer.valueOf(request.getParameter("sortId"));		
		String isInsert = request.getParameter("isInsert");
        if(!orgSortId.equals(sortId)&&isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgPost.class.getSimpleName(), "sortId",sortId,post.getId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgPost.class.getSimpleName(), CurrentUser.get().getLoginAccount(), sortId);
		} 		
		
		 //集团管理员登录修改基准岗后各单位都更新
		if(user.isGroupAdmin()){
			List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_BENCHMARK_POST,"objectiveId",post.getId());
			List<V3xOrgPost> posts = new ArrayList<V3xOrgPost>();
			for(V3xOrgRelationship rel:rels){
				V3xOrgPost accountPost = orgManagerDirect.getPostById(rel.getSourceId());
			...
			if(postBeforeUpdate.getEnabled()&&!model.getEnabled()){
				for(V3xOrgPost accountPost:posts){
					List<V3xOrgMember> mems = orgManagerDirect.getMembersByPost(accountPost.getId());
        ...
        //记录日志
		appLogManager.insertLog(user, AppLogAction.Organization_UpdatePost, user.getName(),post.getName());
	...
	public ModelAndView destroyPost(HttpServletRequest request,
		...
		if (null != ids && !ids.equals("")) {
			...
			if (null != arrIDs && arrIDs.length > 0) {
				List<V3xOrgPost> postList = new ArrayList<V3xOrgPost>(arrIDs.length);
				for (int i=0;i<arrIDs.length;i++) {
					...
					if(user.isGroupAdmin()){
						...
						for(V3xOrgRelationship rel:rels){
							List<V3xOrgMember> mems = orgManagerDirect.getMembersByPost(rel.getSourceId());
						...
						if(!enable2Del) {
							PrintWriter out = response.getWriter();
							out.println("<script>");
							out.println("alert('"+ Constant.getString("organization.delete.member",local)+"');");						
							out.println("</script>");
							out.flush();
							return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Post", "parent");
						} else {
						...
						/*for(V3xOrgRelationship rel:rels){
							...
							orgManagerDirect.deleteEntity(V3xOrgPost.class, rel.getSourceId(), rel.getOrgAccountId());
						}*/
					}else{
						List<V3xOrgMember> members = orgManagerDirect.getMembersByPost(id);
						boolean isAllMemberUnEnabled = true;					
						for(V3xOrgMember mem:members){
							if(mem.isValid()){
								isAllMemberUnEnabled = false;
						...
						if(ListUtils.EMPTY_LIST.equals(members) || isAllMemberUnEnabled){
							...
							//集团基准岗删除关联关系
							/*if(postType.equals(String.valueOf(2))){
								List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_BENCHMARK_POST,"sourceId",id);
                ...
                //记录日志
				appLogManager.insertLogs(user, AppLogAction.Organization_DeletePost, appLogs);
			}
		}
	...
	public ModelAndView listLevel(HttpServletRequest request,
		...
		if(condition!=null && !"".equals(condition)){
			levellist = orgManager.getAllLevels(user.getLoginAccount(),condition,textfield);
		}else{
		...
		for(V3xOrgLevel level:levellist){
			...
			if(level.getGroupLevelId()!=null){
				V3xOrgLevel levelFor = orgManagerDirect.getLevelById(level.getGroupLevelId());		
				if(levelFor!=null){
		...
		//是否显示集团职务级别映射
		V3xOrgAccount account = orgManagerDirect.getAccountById(CurrentUser.get().getLoginAccount());
		if(Boolean.valueOf(SysFlag.org_showGroupLevelMap.getFlag().toString())
				&&account.getSuperior().equals(V3xOrgEntity.DEFAULT_NULL_ID)
				&&!account.getIsRoot()){
			result.addObject("showGroupLevel", false);
		}else{
			result.addObject("showGroupLevel", SysFlag.org_showGroupLevelMap.getFlag());
		}
		result.addObject("condition", condition);
		result.addObject("textfield", textfield);
		return result;
	...
	public ModelAndView listGroupLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		User user = CurrentUser.get();
		ModelAndView result = new ModelAndView("organization/level/listGroupLevel");
		List<V3xOrgLevel> levellist = orgManagerDirect.getAllLevels(user.getLoginAccount());
	...
	public ModelAndView queryGroupLevelByCondition(HttpServletRequest request,
		...
		ModelAndView view = new ModelAndView("organization/level/listGroupLevel");
		String condition = request.getParameter("condition") ;
		String textfield = request.getParameter("textfield");
...
//		String hql = "select a from "+V3xOrgAccount.class.getName()+" a where a.superior = -1 and a.isRoot=1";
//		List<V3xOrgAccount> voaList = searchManager.searchByHql(hql, null, false);
//		if (!voaList.isEmpty()) {
//			V3xOrgAccount account = voaList.get(0);
		User user = CurrentUser.get();
			// 按名称查询
			if (condition != null && "name".equals(condition)) {
				sbHQL.append("select v from ").append(V3xOrgLevel.class.getName()).append(" v where v.isDeleted=0 and v.orgAccountId=:accountId");
			...
			// 根据是否启用查询
				...
				sbHQL.append("select v from ").append(V3xOrgLevel.class.getName()).append(" v where v.enabled=").append(enabled).append(" and v.isDeleted=0 and v.orgAccountId=:accountId");
				view.addObject("condition", condition);
...
//		}
		...
		if (sbHQL.toString() != null)
			levelList = searchManager.searchByHql(sbHQL.toString(), param, false);
		...
		if(Boolean.valueOf(SysFlag.org_showGroupLevelMap.getFlag().toString())
			...
			view.addObject("showGroupLevel", SysFlag.org_showGroupLevelMap.getFlag());
		}
...
//		ModelAndView result = new ModelAndView("organization/level/listLevel");
		...
		DataRecord dataRecord = OrganizationHelper.exportLevel(request, response, 
				fileToExcelManager, orgManagerDirect);
		OrganizationHelper.exportToExcel(request, response, fileToExcelManager, level_list+"-"
				+ user.getLoginName(), dataRecord);
		return null;
	}
	...
	public ModelAndView addLevel(HttpServletRequest request,
		...
		//取得集团职务级别
		List<V3xOrgLevel> groupLevellist = orgManagerDirect.getAllLevels(orgManagerDirect.getRootAccount().getId(),false);
		//过滤无效的级别
	...
	public ModelAndView addGroupLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/level/addGroupLevel");
		V3xOrgLevel level = new V3xOrgLevel();
		level.setEnabled(true);
		result.addObject("level", level);
		return result;
	...
	public ModelAndView createLevel(HttpServletRequest request,
		...
		if(StringUtils.isNotBlank(groupLevelIdStr)){
			groupLevelId = orgManagerDirect.getLevelById(level.getGroupLevelId()).getLevelId();
		}
		boolean isMapRight = orgManagerDirect.isGroupLevelMapRight(user.getLoginAccount(), level.getLevelId(), groupLevelId);
		if(!isMapRight){	
			V3xOrgLevel errorLevel = orgManagerDirect.getErrorMapLevel(user.getLoginAccount(), level.getLevelId(), groupLevelId);
			if(errorLevel.getLevelId().intValue()>level.getLevelId().intValue()){
				out.println("<script>");
				//branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------Start
		        	out.println("alert('"+ Constant.getString("level.map.group.low"+Functions.suffix(),local,errorLevel.getName())+"');");
		        //branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------End
				out.println("</script>");
			}else{
				out.println("<script>");
				//branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------Start
					out.println("alert('"+ Constant.getString("level.map.group.up"+Functions.suffix(),local,errorLevel.getName())+"');");
				//branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------End
				out.println("</script>");
			}
			super.rendJavaScript(response, "parent.toEditMember()");
			return redirectModelAndView("/organization.do?method=organizationFrame&from=Level&addOrganiza=addLevel&cont="+continues);			 
		}else{
            ...
            //记录日志
			appLogManager.insertLog(user, AppLogAction.Organization_NewLevel, user.getName(),level.getName());
			return null;
		}		
	}
	...
	public ModelAndView editGroupLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/level/editGroupLevel");
		String id = request.getParameter("id");
		V3xOrgLevel level = orgManagerDirect.getLevelById(Long.parseLong(id));
		result.addObject("level", level);
		// 取得是否是详细页面标志
	...
	public ModelAndView updateLevel(HttpServletRequest request,
		...
		Integer groupLevelId = V3xOrgEntity.MAX_LEVEL_NUM;
		if(StringUtils.isNotBlank(groupLevelIdStr)){
			groupLevelId = orgManagerDirect.getLevelById(model.getGroupLevelId()).getLevelId();
		}
		boolean isMapRight = orgManagerDirect.isGroupLevelMapRight(CurrentUser.get().getLoginAccount(), model.getLevelId(), groupLevelId);
		if(!isMapRight){
			/*V3xOrgLevel errorLevel = orgManagerDirect.getErrorMapLevel(CurrentUser.get().getLoginAccount(), model.getLevelId(), groupLevelId);
			if(errorLevel.getLevelId().intValue()>level.getLevelId().intValue()){
				out.println("<script>");
				out.println("alert('"+ Constant.getString("level.map.group.low",local,errorLevel.getName())+"');");
				out.println("</script>");
			}else{
				out.println("<script>");				
				out.println("alert('"+ Constant.getString("level.map.group.up",local,errorLevel.getName())+"');");
				out.println("</script>");
			}
			super.rendJavaScript(response, "parent.toEditMember()");
			return null;*/
			...
			if(errorLevel.getLevelId().intValue()>level.getLevelId().intValue()){
				//branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------Start
		        str="alert('"+ Constant.getString("level.map.group.low"+Functions.suffix(),local,errorLevel.getName())+"');";
				
			}else{
				//branches_a8_v350_r_gov GOV-1855 lijl添加判断,如果是政务版本,则以政务的提示信息提示,否则以原来A8的信息提示----------Start
	        	str="alert('"+ Constant.getString("level.map.group.up"+Functions.suffix(),local,errorLevel.getName())+"');";
            ...
            //记录日志
			User user = CurrentUser.get();
			appLogManager.insertLog(user, AppLogAction.Organization_UpdateLevel, user.getName(),level.getName());			
			// 提示用户操作成功
			super.rendJavaScript(response, "parent.doEndLevel()");
			return null;
		}
	...
	public ModelAndView updateGroupLevel(HttpServletRequest request,
		...
		Locale local = LocaleContext.getLocale(request);
        //校验职务级别映射
        List<V3xOrgEntity> levelList = orgManagerDirect.getEntityList(V3xOrgLevel.class.getSimpleName(), "groupLevelId", model.getId(),V3xOrgEntity.VIRTUAL_ACCOUNT_ID);
        for(V3xOrgEntity ent:levelList){
        	V3xOrgLevel levelIt = (V3xOrgLevel)ent;
    		boolean isMapRight = orgManagerDirect.isGroupLevelMapRight(levelIt.getOrgAccountId(), levelIt.getLevelId(), model.getLevelId());
        	if(!isMapRight){
            	V3xOrgLevel errorLevel = orgManagerDirect.getErrorMapLevel(levelIt.getOrgAccountId(), levelIt.getLevelId(), model.getLevelId());
    			if(errorLevel.getLevelId().intValue()>levelIt.getLevelId().intValue()){
    				out.println("<script>");
    				out.println("alert('"+ Constant.getString("level.map.update.group.down",local,errorLevel.getName(),levelIt.getName(),orgManagerDirect.getAccountById(levelIt.getOrgAccountId()).getShortname())+"');");
    				out.println("</script>");
    			}else{
    				out.println("<script>");				
    				out.println("alert('"+ Constant.getString("level.map.update.group.up",local,errorLevel.getName(),levelIt.getName(),orgManagerDirect.getAccountById(levelIt.getOrgAccountId()).getShortname())+"');");
    				out.println("</script>");
    			}
    			super.rendJavaScript(response, "parent.doEndLevel()");
    			return null;
		...
		if (null != ids && !ids.equals("")) {
			...
			if (null != arrID && arrID.length > 0) {
				...
				for (String strID : arrID) {
					...
					//判断职务级别下是否有成员存在
					List<V3xOrgEntity> members = orgManagerDirect.getEntityList(V3xOrgMember.class.getSimpleName(), "orgLevelId", id, level.getOrgAccountId());
					boolean isAllMemberUnEnabled = true;					
					...
					if(ListUtils.EMPTY_LIST.equals(members)||isAllMemberUnEnabled){
						...
						out.println("</script>");
						out.flush();
						return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Level", "parent");
					}				
				}
                ...
                //记录日志
				appLogManager.insertLogs(user, AppLogAction.Organization_DeleteLevel, appLogs);
			}
		}
		// 提示用户操作成功
		PrintWriter out = response.getWriter();
	...
	@CheckRoleAccess(roleTypes={RoleType.Administrator,RoleType.HrAdmin,RoleType.DepartmentAdmin})
	public ModelAndView banchEdit(HttpServletRequest request,
        ...
        if(deptAdmin!=null&&deptAdmin.equals("1")){
            result.addObject("depsPathStr", getDeptPathsForAdmin(CurrentUser.get().getId()));       	
        }
	...
	public ModelAndView showMemberList(HttpServletRequest request,
		...
		if("dept".equals(request.getParameter("dept"))){
			if(strId!=null && strId.length()!=0 && accountId!=null && accountId.length()!=0){
				memberlist = OrganizationHelper.searchMember("orgDepartmentId", strId, searchManager, orgManagerDirect, false, true, false);
			}
		}else{
			if(accountId!=null && accountId.length()!=0){
				memberlist = OrganizationHelper.searchMember(null, null, searchManager, orgManagerDirect, false, true, false);
			}
		}
		...
		if (null != memberlist) {
			for (Object memberObj : memberlist) {
				...
				if(!member.isValid()) continue;//Fix AEIGHT-5978 2012-05-10 lilong 修改BUG点击某部门不显示部门下的人员列表将离职、停用等无效人员剔除出列表
	...
	private ModelAndView doListMember(HttpServletRequest request,
		...
		try {
			...
			if(deptAdmin!=null&&deptAdmin.equals("1")){
				...
				List<V3xOrgMember> mems = OrganizationHelper.searchMember(condition, textfield, searchManager, orgManagerDirect, isOnlyEnableMem, true,true);
				List<V3xOrgMember> memberlistForDeptAdmin = new ArrayList<V3xOrgMember>();
				V3xOrgRole depAdminRole = orgManager.getRoleByName(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN);
				List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_MEMBER_DEPROLE, "backupId", depAdminRole.getId(),"sourceId",user.getId());
				if(rels!=null&&rels.size()>0){
					for(V3xOrgRelationship rel:rels){
						List<V3xOrgMember> deptMembers = orgManager.getMembersByDepartment(rel.getObjectiveId(), false, !isOnlyEnableMem, rel.getOrgAccountId());
						if(deptMembers!=null&&deptMembers.size()>0){
							for(V3xOrgMember deptMem:deptMembers){
								if(!memberlistForDeptAdmin.contains(deptMem)&&deptMem.getIsInternal()){
									if(isOnlyEnableMem) {
										if(deptMem.isValid()) memberlistForDeptAdmin.add(deptMem);
									} else {
				...
				memberlist = OrganizationHelper.searchMember(type,condition, textfield, searchManager, orgManagerDirect, true, true,true);
			}
	...
	public ModelAndView downloadTemplate(HttpServletRequest request,
    	...
    	if(type.equals(V3xOrgEntity.ORGENT_TYPE_MEMBER)){
    		...
    		boolean isGovVersion = (Boolean)SysFlag.is_gov_only.getFlag();
    		if(isGovVersion){
    			path = SystemEnvironment.getA8ApplicationFolder() + "/apps_res/edoc/file/orgnization/memberGov";
    		}else{
    			path = SystemEnvironment.getA8ApplicationFolder() + "/apps_res/edoc/file/orgnization/member";
    		}
    		filename = URLEncoder.encode("member.xls", "UTF-8");
    		
    	}else if(type.equals(V3xOrgEntity.ORGENT_TYPE_POST)){
    		path = SystemEnvironment.getA8ApplicationFolder() + "/apps_res/edoc/file/orgnization/post";
	...
	public ModelAndView queryMember(HttpServletRequest request,
		...
		result.addObject("levellist", levellist);
		// 取得所有的部门
		List<V3xOrgEntity> departmentlist = orgManagerDirect.getEntityListNoRelation(V3xOrgDepartment.class.getSimpleName(), "orgAccountId", user.getLoginAccount(), user.getLoginAccount());
		...
		try {
			...
			if(deptAdmin!=null&&deptAdmin.equals("1")){		
				isdeptAdmin = true;
				V3xOrgRole depAdminRole = orgManagerDirect.getRoleByName(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN);
				List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_MEMBER_DEPROLE, "backupId", depAdminRole.getId(),"sourceId",user.getId());
				if(rels!=null&&rels.size()>0){
					for(V3xOrgRelationship rel:rels){
						List<V3xOrgMember> deptMembers = orgManagerDirect.getMembersByDepartment(rel.getObjectiveId(), false, null, user.getLoginAccount());
						if(deptMembers!=null&&deptMembers.size()>0){
							for(V3xOrgMember deptMem:deptMembers){
			...
			//根据条件查询（部门管理员不需要分页）
			if(deptAdmin!=null&&deptAdmin.equals("1")&&"allmember".equals(type)){
				...
				memberlist = OrganizationHelper.searchMember(condition, textfield, searchManager, orgManagerDirect, !isdeptAdmin, true,true);
			}
...
//		Locale locale = user.getLocale();
		...
		String listname = 
			"MemberList_";
			//ResourceBundleUtil.getString(resource, locale, "org.member_form.list");		
		listname+=u.getLoginName();
		
		String key=null;
		DataUtil.putImpExpAction(u.getId(), "export");
		try{
			DataRecord dataRecord = OrganizationHelper.exportMember(request, metadataManager, 
	...
	public ModelAndView exportMember1(HttpServletRequest request,
		...
		try{
			dataRecord = OrganizationHelper.exportMember(request, metadataManager, 
					response, fileToExcelManager, orgManagerDirect, searchManager, condition, textfield);
		}catch(Exception e){
			DataUtil.removeImpExpAction(u.getId());
			throw e;
		}
		DataUtil.removeImpExpAction(u.getId());
		OrganizationHelper.exportToExcel(request, response, fileToExcelManager, listname, dataRecord);		
		return null;
	}
	...
	public ModelAndView addMember(HttpServletRequest request,
		...
		String deptAdmin = request.getParameter("deptAdmin");
		//获得当前排序号的最大值
		Integer maxSortNum = orgManagerDirect.getMaxMemberSortNum(CurrentUser.get().getLoginAccount());
		WebV3xOrgMember webMember = new WebV3xOrgMember();
		V3xOrgMember member = new V3xOrgMember();
		...
		result.addObject("orgMeta", orgMeta);
		// 获取首选语言的下拉列表
		Map<String, Metadata> globalMeta = metadataManager.getMetadataMap(ApplicationCategoryEnum.global);
        ...
        //Long currentSpaceId = spaceManager.getPersonalSpaceId4Create(true,CurrentUser.get().getLoginAccount());
        //currentSpaceId
	...
	private String getDeptPathsForAdmin(Long userId) throws BusinessException {
		V3xOrgRole role = orgManagerDirect.getRoleByName(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN);
		List<V3xOrgEntity> depts = orgManagerDirect.getGroupByMemberAndRole(userId, role.getId());
		String depsPathStr = "";
	...
	class SingleMemberBuilder implements MemberBuilder
	{
		...
		public boolean check(V3xOrgMember member) throws BusinessException {
			if(checkDepartment(member.getOrgDepartmentId())) return false;
			if(member.getIsInternal())
			{
				if(checkPost(member.getOrgPostId())) return false;
				if(checkLevel(member.getOrgLevelId())) return false;
			}
			return checkSecondPost(member);
			
		}

		public boolean checkSecondPost(V3xOrgMember member) throws BusinessException {
		...
		private boolean checkEntityEnabled(long id, String entityClassName) throws BusinessException {
			List<V3xOrgEntity> entities =  orgManagerDirect.getEntityListNoRelation(entityClassName,"id",id,V3xOrgEntity.VIRTUAL_ACCOUNT_ID);
			if((entities==null)||(entities.size()==0)) return false;
			if(entities.size()>1) throw new BusinessException("find more than one entity:" +entityClassName +" " + id);
		...
		//政务版--职级检查
		public boolean checkDutyLevel(long dutyLevelId) throws BusinessException {
			// V3xOrgLevel level = orgManagerDirect.getLevelById(levelId);
			// return !(level==null||!level.getEnabled());
		...
		public boolean hasDuplicatedSortId(long sortId) throws BusinessException {
			return orgManagerDirect.isPropertyDuplicated(V3xOrgMember.class.getSimpleName(), "sortId", sortId);
		}
		...
		public boolean buildWithNoCheck(V3xOrgMember member) throws BusinessException {
			return false;
		}
	}
	...
	public ModelAndView createMember(HttpServletRequest request,
		...
		try {
			...
			User user = CurrentUser.get();
			long accountId = user.getLoginAccount();
			long deptId = Long.parseLong(request.getParameter("orgDepartmentId"));
            ...
            if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
            {
            	if(Strings.isBlank(ldapEntry)&&Strings.isBlank(selectOU)&&(event.isDefaultOUNull(accountId)==null)){
            		out.println("<script>");
    				out.println("alert(\""+ResourceBundleUtil.getString(LDAPConfig.LDAP_RESOURCE_NAME,"ldap.alert.setdn","")+"\");");
    				out.println("</script>");
    				super.rendJavaScript(response, "parent.toEditMember()");
    				return null;
            	}
			...
			if(member.getSecretLevel() != null && member.getSecretLevel() != SecretAudit.SECRET_LEVEL_NORMAL) {
                secretAuditManager.create(member);
                // 新建时人员密级默认为非密
                member.setSecretLevel(SecretAudit.SECRET_LEVEL_NORMAL);
            }
			...
			if (null != arrSecondPosts && arrSecondPosts.length > 0) {
				for (String secondpostid : arrSecondPosts) {
					if(secondpostid.indexOf("_")!=-1){
						...
						if(StringUtils.isNotBlank(arrDeptPosts[0])&&StringUtils.isNotBlank(arrDeptPosts[1])){
			...
			if(!builder.checkDepartment(deptId)){
				out.println("<script>");
				out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_unabled_property','"+Constant.getString4CurrentUser("org.member_form.deptName.label")+"'));");
				out.println("</script>");
				super.rendJavaScript(response, "parent.toEditMember()");
				return null;
			}
			if(member.getIsInternal()){
				...
				if(!builder.checkPost(postId)){
					out.println("<script>");
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_unabled_property','"+Constant.getString4CurrentUser("org.member_form.primaryPost.label")+"'));");
					out.println("</script>");
					super.rendJavaScript(response, "parent.toEditMember()");
					return null;
				}		
				if(!builder.checkLevel(levelId)){
					out.println("<script>");
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_unabled_property','"+Constant.getString4CurrentUser("org.member_form.levelName.label")+"'));");
					out.println("</script>");
					super.rendJavaScript(response, "parent.toEditMember()");
					return null;
				}
				
				if(isGovVersion && request.getParameter("orgDutyLevelId")!=null){
					long dutyLevelId = Long.parseLong(request.getParameter("orgDutyLevelId"));	
					if(!builder.checkDutyLevel(dutyLevelId)){
						out.println("<script>");
						out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_unabled_property','"+Constant.getString4CurrentUser("org.dutylevel_form.name.label")+"'));");
						out.println("</script>");
						super.rendJavaScript(response, "parent.toEditMember()");
						return null;
					}
            ...
            if(isInsert.equals("1")&& orgManagerDirect.isPropertyDuplicated(V3xOrgMember.class.getSimpleName(), "sortId", member.getSortId())){
            	orgManagerDirect.insertRepeatSortNum(V3xOrgMember.class.getSimpleName(),accountId, member.getSortId());
			}			
...
//			blogManager.createEmployee(member.getId(), accountId);
			...
			if(isGovVersion){
				...
				if(political_position!=null){
					staffinfo.setPolitical_position(Integer.parseInt(political_position));
				}
				//最高学位
				if(degreeLevel!=null){
            ...
            if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
            {
                try
                {
                    if(StringUtils.isBlank(ldapEntry))
                    {
                        ...
                        appLogManager.insertLog(user, AppLogAction.LDAP_Account_Create,  newMember.getName(),selectOU);
                    }
                    else
                    {
                        String[] errorResult=event.addMember(newMember,ldapEntry);
                        ...
                        appLogManager.insertLog(user, AppLogAction.LDAP_Account_Bing_Create,  newMember.getName(),ldapEntry);
                    }
                }
                catch (Exception e)
                {
...
//                   throw new BusinessException("ldap/ad 添加人员绑定不成功！",e);
                   if(e.getMessage().indexOf("error code 19")!=-1){
                	   out.println("<script>");
                       out.println("alert('"+ResourceBundleUtil.getString(LDAPConfig.LDAP_RESOURCE_NAME,"ldap.log.error.tip")+"');");
                       out.println("</script>");
                   }
                }
            }
			...
			if(!Strings.isBlank(type)){
				...
				Metadata metadata = metadataManager.getMetadata(MetadataNameEnum.org_property_member_type);
				if(null!=metadata){
					MetadataItem item = metadataManager.getMetadataItem(metadata.getName(), Long.valueOf(iad).toString());
					if(null!=item){
						metadataManager.refMetadataItem(metadata.getId(), item.getId(), Long.valueOf(iad).intValue());
			...
			if(!Strings.isBlank(state)){
				...
				Metadata metadata = metadataManager.getMetadata(MetadataNameEnum.org_property_member_state);
				if(null!=metadata){
					MetadataItem item = metadataManager.getMetadataItem(metadata.getName(), Long.valueOf(iad).toString());
					if(null!=item){
						metadataManager.refMetadataItem(metadata.getId(), item.getId(), Long.valueOf(iad).intValue());
...
//            this.templeteConfigManager.pushNewOrgEntityTemplete4Member(V3xOrgEntity.ORGENT_TYPE_MEMBER,member.getId(),member.getId());
			...
			//日志
			appLogManager.insertLog(user, AppLogAction.Organization_NewMember, user.getName(),member.getName());
			V3xOrgMember memberNext = new V3xOrgMember();
			super.rendJavaScript(response, "parent.doEndMemberFrom('"+accountId+"','addMember','"+memberNext.getId()+"')");
			return null;
		} catch (BusinessException e) {
	...
	public ModelAndView createMemberOld(HttpServletRequest request,
		...
		try {
				...
				for (String secondpostid : arrSecondPosts) {
					...
					if(arrDeptPosts.length>1){
						if (null != arrDeptPosts[0] && !arrDeptPosts[0].equals("")
		                    ...
		                    //校验副岗是否和主岗重复
							if(secondDeptId.equals(member.getOrgDepartmentId())&&secondPostId.equals(member.getOrgPostId())){
								out.println("<script>");
								out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_member_secondPost_repeat'));");
								out.println("</script>");
								//后续操作
			...
			super.rendJavaScript(response, "parent.doEndMemberFrom('"+accountId+"','addMember','')");
	...
	public ModelAndView editMember(HttpServletRequest request,
		...
		//取得人员兼职信息
		List cntList = orgManagerDirect.getAllConcurrentPostByMemberId(member.getId());
        ...
        if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
        {
            ...
            try
            {
                result.addObject("ldapADLoginName", event.getLdapAdExUnitCode(member.getLoginName()));
                result.addObject("hasLDAPAD", true);
	...
	public ModelAndView updateBanchMember(HttpServletRequest request,
		...
		UpdateBanchMemberEvent updateBanchMemberEvent=new UpdateBanchMemberEvent(this);
		...
		if (msg.isSuccess()) {
			...
			sb.append("alert(\'");
			sb.append(
					StringEscapeUtils.escapeJavaScript(ResourceBundleUtil.getString(
							resourceBundle, "batchedit.failure.members")))
					.append("\\n");
...
/*
	...
	public ModelAndView updateBanchMember(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		//马传佳写的这部分代码需要细查，可能存在隐患
		PrintWriter out = response.getWriter();
		String idsString = request.getParameter("ids");
		StringBuffer sbrPost = new StringBuffer();
		StringBuffer sbrForm = new StringBuffer();
		if(idsString!=null && idsString!=""){
			...
			String [] idsArry = idsString.split(",");
			List<V3xOrgEntity> members = new ArrayList<V3xOrgEntity>();
			for(int i=0;i<idsArry.length;i++){
				request.setAttribute("id", idsArry[i]);
				V3xOrgMember member = orgManagerDirect.getMemberById(Long.valueOf(idsArry[i]));
				members.add(member);
				// 为事件调用记录修改前的人员
				V3xOrgMember memberBeforeUpdate = new V3xOrgMember();
				BeanUtils.copyProperties(memberBeforeUpdate, member);								
				if(request.getParameter("orgDepartmentId")!=null && request.getParameter("orgDepartmentId").length()!=0){
					member.setOrgDepartmentId(Long.valueOf(request.getParameter("orgDepartmentId")));
					//如果是部门主管，离职人员跳动部门是否允许?--不允许ajax前台判断
				}
				if(request.getParameter("orgLevelId")!=null && request.getParameter("orgLevelId").length()!=0){
					member.setOrgLevelId(Long.valueOf(request.getParameter("orgLevelId")));
				}
				if(request.getParameter("orgPostId")!=null && request.getParameter("orgPostId").length()!=0){
					List<MemberPost> memberpost = member.getSecond_post();
					//主岗
					boolean f = false;
					for(MemberPost secPost:memberpost){
						if(secPost.getPostId().equals(Long.valueOf(request.getParameter("orgPostId")))){
							if(secPost.getDepId().equals(member.getOrgDepartmentId())){
								f=true;
								break;
							}
						}
					...
					if(!f){
						...
						sbrPost.append(member.getName()).append(",");
					}
				...
				if(request.getParameter("type")!=null && request.getParameter("type").length()!=0){
					member.setType(Byte.valueOf(request.getParameter("type")));
				}
				// 人员状态
				if (null != state) {
					member.setState(state);
				}
				if(request.getParameter("primaryLanguange")!=null && request.getParameter("primaryLanguange").length()!=0){
			...
			if(request.getParameter("orgLevelId")!=null && request.getParameter("orgLevelId").length()!=0){
				properties.put("orgLevelId",Long.valueOf(request.getParameter("orgLevelId")));
			}
			if(request.getParameter("orgDepartmentId")!=null && request.getParameter("orgDepartmentId").length()!=0){
				properties.put("orgDepartmentId",Long.valueOf(request.getParameter("orgDepartmentId")));
			}		
			if(request.getParameter("orgPostId")!=null && request.getParameter("orgPostId").length()!=0){
				properties.put("orgPostId",Long.valueOf(request.getParameter("orgPostId")));
			}
			if(request.getParameter("gender")!=null && request.getParameter("gender").length()!=0){
				properties.put("gender",Integer.parseInt(request.getParameter("gender")));
			}
			if(request.getParameter("type")!=null && request.getParameter("type").length()!=0){
			...
			if(request.getParameter("primaryLanguange")!=null && request.getParameter("primaryLanguange").length()!=0){
				properties.put("primaryLanguange",request.getParameter("primaryLanguange"));
			}		
			// 账户状态
			if (null != enabled) {
				properties.put("enabled", enabled);
			}
			orgManagerDirect.updateEntitys(members,properties);
		}
		//记录日志
		User user = CurrentUser.get();
		appLogManager.insertLog(user, AppLogAction.Organization_BanchEditMember, user.getName());
		out = response.getWriter();
		out.println("<script>");
		if(sbrPost.toString().length()>0){
			out.println("alert(\'"+sbrPost+"\' +parent.v3x.getMessage('organizationLang.orgainzation_brother_post_name'));");
		}
		if(sbrForm.toString().length()>0){
			out.println("alert(\'"+sbrForm+"\'+parent.v3x.getMessage('organizationLang.orgainzation_brother_form_name'));");
		}
		out.println("window.returnValue='true';");
		out.println("window.close();");
		out.println("</script>");
	...
	public ModelAndView updateMember(HttpServletRequest request,
		...
		if(staffinfo==null){
			...
			if(isGovVersion){
				...
				//政治面貌
				if(political_position!=null){
					staff.setPolitical_position(Integer.parseInt(political_position));
				}
				//最高学位
				if(degreeLevel!=null){
		...
		Integer sortId = Integer.valueOf(request.getParameter("sortId"));		
		String isInsert = request.getParameter("isInsert");
        if(!orgSortId.equals(sortId)&&isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgMember.class.getSimpleName(), "sortId", sortId,member.getId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgMember.class.getSimpleName(), CurrentUser.get().getLoginAccount(), sortId);
		}
		...
		if(password!=null
				...
				&&!isLoginNameModifyed){
			appLogManager.insertLog(user, AppLogAction.Systemmanager_UpdateUserPassWord, user.getName(),member.getName());
		}
...
//		addDeptPost(member);
		...
		//日志
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateMember, user.getName(),member.getName());
		if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
        {
            try
            {
                ...
                if(isLoginNameModifyed)
             {
              ...
              if(event.getLdapAdExUnitCode(model.getLoginName()).equals(ldapEntry)&&!isLoginNameModifyed&&!com.seeyon.v3x.common.ldap.config.LDAPConfig.getInstance().isDisabledModifyPassWord()
  					&&!StringUtils.isEmpty(password)
					&&!password.equals(V3xOrgEntity.DEFAULT_INTERNAL_PASSWORD))
              {
            	  event.changePassword(memberBeforeUpdate, memberLdap);
            	  appLogManager.insertLog(user, AppLogAction.LDAP_PassWord_Update, memberLdap.getName(),ldapEntry);
              }else if(!event.getLdapAdExUnitCode(memberLdap.getLoginName()).equals(ldapEntry))
              {
            	  String[] errorResult=event.addMember(memberLdap,ldapEntry);
            	  ...
            	  appLogManager.insertLog(user, AppLogAction.LDAP_Account_Bing_Create, memberLdap.getName(),ldapEntry);
            			if(!com.seeyon.v3x.common.ldap.config.LDAPConfig.getInstance().isDisabledModifyPassWord()
            					&&!StringUtils.isEmpty(password)
            					&&!password.equals(V3xOrgEntity.DEFAULT_INTERNAL_PASSWORD)){
            				event.changePassword(memberBeforeUpdate, memberLdap);
            				appLogManager.insertLog(user, AppLogAction.LDAP_PassWord_Update, memberLdap.getName(),ldapEntry);
	 ...
	 */
	@CheckRoleAccess(roleTypes={RoleType.Administrator, RoleType.HrAdmin, RoleType.DepartmentAdmin})
	public ModelAndView cancelMember(HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		V3xOrgMember member = new V3xOrgMember();
		PrintWriter out = response.getWriter();
		boolean hasLdapPlugin = LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name());
		List<V3xOrgMember>  memberList=new ArrayList<V3xOrgMember>();
		for (String string : membetIds) {
			...
			try {
				...
				if (isBulAuditor) {
					out = response.getWriter();
					out.println("<script>");				
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_move_member_public_information_auditor_bul','"+member.getName()+"'));");
					out.println("</script>");	
					out.flush();
					return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin, "parent");
				}
				if (isInquiryAuditor) {
					out = response.getWriter();
					out.println("<script>");				
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_move_member_public_information_auditor_inquiry','"+member.getName()+"'));");
					out.println("</script>");	
					out.flush();
					return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin, "parent");
				}
				if (isNewsAuditor) {
					out = response.getWriter();
					out.println("<script>");				
					out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_move_member_public_information_auditor_news','"+member.getName()+"'));");
					out.println("</script>");	
					out.flush();
					return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin, "parent");
				}
		...
		List<String[]> appLogs = new ArrayList<String[]>();
        User user = CurrentUser.get();
        V3xOrgAccount account = orgManagerDirect.getAccountById(user.getAccountId());
        ...
        appLogManager.insertLogs(user, AppLogAction.Organization_MoveMember, appLogs);
        ...
        if(successNum == 0){
            ...
            out.println("<script>");
            out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_delete_someMember_pendingaffair','"+faileMemberNames+"'));");
            out.println("</script>");
            out.flush();
        }
		return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin, "parent");	
	...
	public ModelAndView destroyMember(HttpServletRequest request,
		...
		List<V3xOrgMember> memberList = new ArrayList<V3xOrgMember>(membetIds.length);
		boolean hasLdapPlugin = LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name());
		for (String string : membetIds) {
			Long ids = Long.parseLong(string);
			// 为事件调用取人员实体
			...
			if (isBulAuditor || isInquiryAuditor || isNewsAuditor) {
				out = response.getWriter();
				out.println("<script>");				
				out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_delete_member_public_information_auditor','"+member.getName()+"'));");
				out.println("</script>");	
				out.flush();
				return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin+"&isTree="+isTree, "parent");
			}
...
//			projectManager.retakeClew(ids);//如果删除的人是项目管理员。那么给发起者发消息				
			...
			/*if(signetManager.hasSignet(ids))
			{
				out = response.getWriter();
				out.println("<script>");				
				out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_delete_member_has_signet','"+member.getName()+"'));");
				out.println("</script>");	
				out.flush();
				return super.redirectModelAndView("/organization.do?method=organizationFrame&from=Member&deptAdmin="+deptAdmin, "parent");				
			}*/
			...
			OnlineRecorder.moveToOffline(member.getLoginName(), LoginOfflineOperation.adminKickoff);
			//删除代理信息
			agentIntercalateManager.cancelUserAgent(member.getId(),CurrentUser.get());
		...
		appLogManager.insertLogs(user, AppLogAction.Organization_DeleteMember, appLogs);
	...
	public ModelAndView listTeam(HttpServletRequest request,
		...
		User user = CurrentUser.get();
		String accountName = orgManagerDirect.getAccountById(user.getLoginAccount()).getName();
		ModelAndView result = null;
		if(user.isGroupAdmin()){
		...
		List<V3xOrgEntity> teamlist = orgManagerDirect.getEntityList(V3xOrgTeam.class.getSimpleName(), "type", V3xOrgEntity.TEAM_TYPE_SYSTEM, Long.valueOf(user.getLoginAccount()),true);
		...
		for (V3xOrgEntity teamEnt : teamlist) {	
				...
				for (Long id : lead) {
					...
					if(vm.getEnabled()&&!vm.getIsDeleted()) leads += (String.valueOf(id)+",");
				}
				...
				// 取得组的部门名称
				V3xOrgEntity dept = orgManagerDirect.getEntityNoRelation(V3xOrgDepartment.class.getSimpleName(), "id", team.getDepId(), user.getLoginAccount());
				if (null != dept) {
					...
					if (department != null && !departments.contains(department)) {
						...
						if (deptTeams != null && deptTeams.size() > 0) {
							for (V3xOrgTeam deptTeam : deptTeams) {
								if (deptTeam.getType()==V3xOrgEntity.TEAM_TYPE_SYSTEM && !teamListByDeptAdmin.contains(deptTeam)) {
	 ...
	 * @throws Exception
	 */
	@CheckRoleAccess(roleTypes={RoleType.Administrator, RoleType.GroupAdmin, RoleType.DepartmentAdmin, RoleType.HrAdmin})
	// && Functions.isRole(V3xOrgEntity.ORGENT_META_KEY_DEPADMIN, CurrentUser.get()) url地址拼接参数，可能还需要这个条件判断
	public ModelAndView addTeam(HttpServletRequest request,
		...
		if (deptAdmin != null && deptAdmin.equals("1")) {
			...
			List<V3xOrgEntity> depts = orgManagerDirect
					.getGroupByMemberAndRole(CurrentUser.get().getId(), role
							.getId());
			String depsPathStr = "";
			for (V3xOrgEntity dept : depts) {
				depsPathStr += ((V3xOrgDepartment) dept).getPath() + "|";
	...
	public ModelAndView createTeam(HttpServletRequest request,
		...
		//加入组重名的校验
		if(orgManagerDirect.isPropertyDuplicated(V3xOrgTeam.class.getSimpleName(), "name", team.getName())){
			V3xOrgTeam dupTeam = (V3xOrgTeam)orgManagerDirect.getEntityList(V3xOrgTeam.class.getSimpleName(), "name", team.getName(), CurrentUser.get().getLoginAccount()).get(0);
			if(dupTeam.getType()==team.getType()){
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert('" + Constant.getString("team.edit.same.name",local)+ "');");	
		...
		//排序号的重复处理
		String isInsert = request.getParameter("isInsert");
        if(isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgTeam.class.getSimpleName(), "sortId", team.getSortId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgTeam.class.getSimpleName(), CurrentUser.get().getLoginAccount(), team.getSortId());
		}
		orgManagerDirect.addTeam(team);
		//日志
		appLogManager.insertLog(user, AppLogAction.Organization_NewTeam, user.getName(),team.getName());
		super.rendJavaScript(response, "parent.doEndTeam('"+CurrentUser.get().getLoginAccount()+"')");
	...
	private void processTeam(V3xOrgTeam team, WebV3xOrgTeam webTeam)
		...
		for (Long ids : supervisors) {
			...
			if(vm.getEnabled()&&!vm.getIsDeleted())supervisorsMembers.add(vm);
		}
		...
		for (Long ids : member) {
			V3xOrgMember vm = orgManagerDirect.getMemberById(ids);
			if(vm.getEnabled()&&!vm.getIsDeleted())memberList.add(vm);
		}
		...
		for (Long ids : lead) {
			V3xOrgMember vm = orgManagerDirect.getMemberById(ids);
			if(vm.getEnabled()&&!vm.getIsDeleted())leadMembers.add(vm);
		}
		...
		for (Long ids : relatives) {
			V3xOrgMember vm = orgManagerDirect.getMemberById(ids);
			if(vm.getEnabled()&&!vm.getIsDeleted())relativesMembers.add(vm);
		}
	...
	public ModelAndView editGroupTeam(HttpServletRequest request,
		...
		if(team.getDepId() != null){
			...
			if (null == dept) {		//有可能是单位	
				V3xOrgAccount account = orgManagerDirect.getAccountById(team.getDepId());
				if(account != null){
		...
		if(dept == null){ //如果没有值，则用当前登录者单位的
			...
			dept.setName(orgManagerDirect.getAccountById(CurrentUser.get().getLoginAccount()).getName());
		}	
	...
	public ModelAndView updateTeam(HttpServletRequest request,
		...
		if(team.getIsDeleted()){
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('" + Constant.getString("team.edit.check.delete",local) + "');");			
			out.println("</script>");				
			super.rendJavaScript(response,"parent.parent.listFrame.location.reload(true);");
			return null;			
		}
		//加入组重名的校验
		if(orgManagerDirect.isPropertyDuplicated(V3xOrgTeam.class.getSimpleName(), "name", model.getName(), model.getId())){
			List<V3xOrgEntity> teamLst = orgManagerDirect.getEntityList(V3xOrgTeam.class.getSimpleName(), "name", model.getName(), CurrentUser.get().getLoginAccount());
			for(V3xOrgEntity dupTeam : teamLst){
				V3xOrgTeam myTeam = (V3xOrgTeam)dupTeam;
				if(!myTeam.getId().equals(model.getId())&&myTeam.getType()==model.getType()){
					PrintWriter out = response.getWriter();
					out.println("<script>");
					out.println("alert('" + Constant.getString("team.edit.same.name",local)+ "');");			
					out.println("</script>");				
		...
		Integer sortId = Integer.valueOf(request.getParameter("sortId"));		
		String isInsert = request.getParameter("isInsert");
        if(!orgSortId.equals(sortId)&&isInsert.equals("1")&&orgManagerDirect.isPropertyDuplicated(V3xOrgTeam.class.getSimpleName(), "sortId", sortId,team.getId())){
        	orgManagerDirect.insertRepeatSortNum(V3xOrgTeam.class.getSimpleName(), CurrentUser.get().getLoginAccount(), sortId);
		}
		...
		//日志
		User user = CurrentUser.get();
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateTeam, user.getName(),team.getName());
		super.rendJavaScript(response, "parent.parent.detailFrame.location.href=\"/seeyon/common/detail.jsp\";parent.parent.listFrame.location.reload(true);");
		return null;
	}
	...
	public ModelAndView destroyTeam(HttpServletRequest request,
		...
		if (null != ids && ids.length() > 0) {
			...
			if (null != arrIDs && arrIDs.length > 0) {
				...
				appLogManager.insertLogs(user, AppLogAction.Organization_DeleteTeam, appLogs);
			}
		}
	...
	public ModelAndView exportPost(HttpServletRequest request,
		...
		DataUtil.putImpExpAction(u.getId(), "export");
		try{
			DataRecord dataRecord = OrganizationHelper.exportPost(request, metadataManager, 
					response, fileToExcelManager, orgManagerDirect);
			key=DataUtil.createTempSaveKey4Sheet(dataRecord);
		}catch(Exception e){
			DataUtil.removeImpExpAction(u.getId());
	...
	public ModelAndView exportPost1(HttpServletRequest request,
		...
		listname+=u.getLoginName();
		**/
		String listname = ResourceBundleUtil.getString(resource, local, "org.post_form.list");
		listname+=u.getLoginName();
		DataUtil.putImpExpAction(u.getId(), "export");
		DataRecord dataRecord=null;
		try{
			dataRecord = OrganizationHelper.exportPost(request, metadataManager, 
					response, fileToExcelManager, orgManagerDirect);
		}catch(Exception e){
			DataUtil.removeImpExpAction(u.getId());
			throw e;
	...
	public ModelAndView exportTeam(HttpServletRequest request,
		...
		DataRecord dataRecord = OrganizationHelper.exportTeam(request, metadataManager, response,
				fileToExcelManager, orgManagerDirect);
		OrganizationHelper.exportToExcel(request, response, fileToExcelManager, team_list+"-"
				+ user.getLoginName(), dataRecord);		
		return null;
	}
...
//		----------------------------------accountSheet---------------------
		...
		String dept_list = ResourceBundleUtil.getString(resource, local, "org.dept_form.list");
		V3xOrgAccount account = orgManagerDirect.getAccountById(user.getLoginAccount());
		List<V3xOrgDepartment> deptlist = orgManagerDirect.getAllDepartments(user.getLoginAccount());
		List<WebV3xOrgDepartment> resultlist = new ArrayList<WebV3xOrgDepartment>();
		for (int i = 0; i < deptlist.size(); i++) {
			V3xOrgDepartment dept = (com.seeyon.v3x.organization.domain.V3xOrgDepartment) deptlist
					.get(i);
		...
		drArray[0] = OrganizationHelper.exportDept(resultlist, request, metadataManager, response, fileToExcelManager, orgManagerDirect, spaceManager, account);

		//----------------------------------deptSheet------------------------
		
		//----------------------------------teamSheet------------------------
		String team_list = ResourceBundleUtil.getString(resource, local, "team.list");
			drArray[1] = OrganizationHelper.exportTeam(request, metadataManager, response, fileToExcelManager, orgManagerDirect);
			//-----------------------------teamSheet-------------------------
			
			//-----------------------------levelSheet------------------------
			String level_list = ResourceBundleUtil.getString(resource, local, "org.level_form.list");
				drArray[2] = OrganizationHelper.exportLevel(request, response, fileToExcelManager, orgManagerDirect);
			//--------------------------levelSheet---------------------------
			
			
			//--------------------------postSheet----------------------------
			String post_list = ResourceBundleUtil.getString(resource, local, "org.post_form.list");
			drArray[3] = OrganizationHelper.exportPost(request, metadataManager, response, fileToExcelManager, orgManagerDirect);
			//-------------------------------postSheet-----------------------
			...
			drArray[4] = OrganizationHelper.exportMember(request, metadataManager, response, fileToExcelManager, orgManagerDirect);			
...
//		根据选择导入文件语言获取对应locale对象
		...
		String permission_all = ResourceBundleUtil.getString(resource, locale, "org.metadata.access_permission.all");
		String permission_up = ResourceBundleUtil.getString(resource, locale, "org.metadata.access_permission.up");
		String permission_upAnddown = ResourceBundleUtil.getString(resource, locale, "org.metadata.access_permission.upAnddown");
		String permission_upAndpar = ResourceBundleUtil.getString(resource, locale, "org.metadata.access_permission.upAndpar");
		String add = ResourceBundleUtil.getString(resource, locale, "import.report.add");
		String overcast = ResourceBundleUtil.getString(resource, locale, "import.report.overcast");
		String overleap = ResourceBundleUtil.getString(resource, locale, "import.report.overleap");
		String fail_accountNameNull = ResourceBundleUtil.getString(resource, locale, "import.account.report.fail.nameNull");
		String fail_accountShortNameNull = ResourceBundleUtil.getString(resource, locale, "import.account.report.fail.shortNameNull");
		String fail_accountAliasNull = ResourceBundleUtil.getString(resource, locale, "import.account.report.fail.aliasNull");
		String fail_accountCodeNameNull = ResourceBundleUtil.getString(resource, locale, "import.account.report.fail.codeNull");
		String fail_accountAdminNameNull = ResourceBundleUtil.getString(resource, locale, "import.account.report.fail.adminNameNull");
		String fail = ResourceBundleUtil.getString(resource, locale, "import.report.fail");
		String company_isRoot_yes = ResourceBundleUtil.getString(resource, locale, "org.account_form.isRoot.yes");
		PrintWriter out = response.getWriter();
		try {
...
//					上级单位
					superior = orgManagerDirect.getAccountByName(proList.get(10));
...
//					获取扩展属性
					List<String> extProList = orgManagerDirect.extPropertyNames(account.getEntityType());
...
//					单位角色指派的标识
					int rolebs = 25;
						...
						while(proList.size()>rolebs){
								...
								if(proList.get(rolebs)!=null&&!proList.get(rolebs).equals("")){
									
									role = orgManagerDirect.getRoleByName(proList.get(rolebs));
									if(role!=null){
										roleList.add(role.getId());
										if(proList.get(rolebs+1)!=null&&!proList.get(rolebs+1).equals("")){
											members = proList.get(rolebs+1).split(",");
											...
											orgManagerDirect.addRole2Member(role.getBond(), account.getId(), role.getId(), memberIds);
										}
									}
								}
								rolebs+=2;
...
//					管理员登录名
					if(proList.get(23)!=null&&!proList.get(23).equals("")){
						V3xOrgMember accountManager = orgManagerDirect.getMemberByLoginName(account.getAdminName());
						accountManager.setLoginName(proList.get(23));
						if(proList.get(24)!=null&&!proList.get(24).equals("")){
...
//			导入文件名
			session.setAttribute("fileName", fileURL.substring(fileURL.lastIndexOf("/")+1));
//			报告数据
...
//		国际化
		String state_Enabled = ResourceBundleUtil.getString(resource, locale, "org.account_form.enable.use");
		String fail_levelNameNull = ResourceBundleUtil.getString(resource, locale, "import.level.report.fail.nameNull");
		String fail_sortIdNull = ResourceBundleUtil.getString(resource, locale, "import.level.report.fail.sortIdNull");
		String fail = ResourceBundleUtil.getString(resource, locale, "import.report.fail");
		String add = ResourceBundleUtil.getString(resource, locale, "import.report.add");
		String overcast = ResourceBundleUtil.getString(resource, locale, "import.report.overcast");
		String overleap = ResourceBundleUtil.getString(resource, locale, "import.report.overleap");
...
//		国际化
		String state_Enabled = ResourceBundleUtil.getString(resource, locale, "org.account_form.enable.use");
		String fail_postNameNull = ResourceBundleUtil.getString(resource, locale, "import.post.report.fail.postNameNull");
		String fail_postTypeNull = ResourceBundleUtil.getString(resource, locale, "import.post.report.fail.postTypeNull");
		String fail = ResourceBundleUtil.getString(resource, locale, "import.report.fail");
		String add = ResourceBundleUtil.getString(resource, locale, "import.report.add");
		String overcast = ResourceBundleUtil.getString(resource, locale, "import.report.overcast");
		String overleap = ResourceBundleUtil.getString(resource, locale, "import.report.overleap");
		...
		for(MetadataItem item : itemList){
			lable = ResourceBundleUtil.getString(resource, locale, item.getLabel());
			if(!lable.equals("")){
...
//					岗位类别
					if(itemMap.get(proList.get(1))!=null&&!itemMap.get(proList.get(1)).equals("")){
...
//		根据选择导入文件语言获取对应locale对象
		...
		String import_report = ResourceBundleUtil.getString(resource, locale, "import.report");
...
//		将导入结果添加到excel中
		...
		try {
			fileToExcelManager.save(request, response, import_report, "location.href", dataRecord);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		return null;
	...
	public ModelAndView doImport(HttpServletRequest request,
		...
		if(selectvalue == null || "".equals(selectvalue) || "null".equals(selectvalue)){
			...
			if(selectvalue.equals("post")){
				appLogManager.insertLog(user, AppLogAction.Organization_BatchAddPost, user.getName());
			}else if(selectvalue.equals("member")){
				appLogManager.insertLog(user, AppLogAction.Organization_BatchAddMember, user.getName());
			}
		}	
...
//		根据选择导入文件语言获取对应locale对象
		...
		List volst = du.getMatchValue(orgManagerDirect,this.metadataManager,Long.valueOf(accountid),accountList, DataObjectlst);
		...
		if(selectvalue.equals("level")){
			...
			orgManagerDirect.reloadOrganizationModel();
			List<V3xOrgDepartment> deptlst = orgManagerDirect.getAllDepartments(Long.valueOf(accountid), false);
...
//					 添加个人文档库
	...
	private void addAccountInitialRole(V3xOrgAccount account)throws Exception {
		...
		//单位角色
		List<MetadataItem> roleNames = metadataManager.getMetadataItems(MetadataNameEnum.organization_account_role);
		...
		//部门角色
		roleNames = metadataManager.getMetadataItems(MetadataNameEnum.organization_department_role);
		for(Iterator<MetadataItem> it = roleNames.iterator(); it.hasNext(); )
		{
			V3xOrgRole	role = new V3xOrgRole();
			MetadataItem mDataIt = it.next();
	...
	public ModelAndView exportOrgReport(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		List<DataRow[]> datarowList = (List<DataRow[]>)session.getAttribute("datarowList");
		String language = (String)session.getAttribute("language");
		String resource = "com.seeyon.v3x.organization.resources.i18n.OrganizationResources";
		Locale locale = null;
...
//		根据选择导入文件语言获取对应locale对象
		...
		String import_organization_report = ResourceBundleUtil.getString(resource, locale, "import.organization.report");
		String import_data = ResourceBundleUtil.getString(resource, locale, "import.data");
		String import_result = ResourceBundleUtil.getString(resource, locale, "import.result");
		String import_description = ResourceBundleUtil.getString(resource, locale, "import.description");
		...
		try {
			fileToExcelManager.save(request, response, import_organization_report , "location.href", dataRecordArray);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		return null;
	...
	@CheckRoleAccess(roleTypes={RoleType.Administrator, RoleType.HrAdmin})
	public ModelAndView departManageInitPage(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ModelAndView mav = new ModelAndView("organization/department/InitPageShow");    
		return mav; 
	}
	...
	public int getMemberPending(Long memId){
		try{
			...
			/*
			Integer[] apps = new Integer[7];
			apps[0] = ApplicationCategoryEnum.collaboration.key();
			apps[1] = ApplicationCategoryEnum.edocSend.key();
			apps[2] = ApplicationCategoryEnum.edocRec.key();
			apps[3] = ApplicationCategoryEnum.edocSign.key();
			apps[4] = ApplicationCategoryEnum.exSend.key();
			apps[5] = ApplicationCategoryEnum.exSign.key();
			apps[6] = ApplicationCategoryEnum.edocRegister.key();
			Set appInts = affairManager.hasPending2(apps, memId).keySet();
			if(!appInts.isEmpty()){
				return (Integer)appInts.toArray()[0];
			}else if(inquiryManager.hasInquiryNoCheck(memId)){
				return ApplicationCategoryEnum.inquiry.key();
			}else if(bulDataManager.hasPendingOfUser(memId)){
				return ApplicationCategoryEnum.bulletin.key();
			}else if(newsDataManager.hasPendingOfUser(memId)){
				return ApplicationCategoryEnum.news.key();
			}else if(iOperBase.queryOwnerListByownerid(memId)){
				return ApplicationCategoryEnum.form.key();
			}else if(officeAdminInfoTarget.hasAdminInUse(memId)){
				return ApplicationCategoryEnum.office.key();
			}	
			*/	
	...
	public ModelAndView externalHome(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/externalHome");
		return modelAndView;
	}
	...
	public ModelAndView externalHomeEntry(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/externalHomeEntry");
		String settingType = request.getParameter("settingType");
		if(!StringUtils.isEmpty(settingType)){
			modelAndView.addObject("settingType", request.getParameter("settingType"));
		}else{
	...
	public ModelAndView listExternalDept(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/listExternalDept");
		List<V3xOrgEntity> externalDepts = orgManagerDirect.getEntityListNoRelation(V3xOrgDepartment.class.getSimpleName(), "isInternal", false, CurrentUser.get().getLoginAccount(),true);
		List<WebV3xOrgDepartment> resultlist = new ArrayList<WebV3xOrgDepartment>();
		for (V3xOrgEntity externalDept:externalDepts) {
			V3xOrgDepartment dept = (V3xOrgDepartment)externalDept;
			V3xOrgDepartment parent = orgManagerDirect.getParentDepartment(dept.getId());
			WebV3xOrgDepartment webdept = new WebV3xOrgDepartment();
			webdept.setV3xOrgDepartment(dept);
			if (null != parent) {
				webdept.setParentId(parent.getId());
	...
	public ModelAndView addExternalDept(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/addExternalDept");
		WebV3xOrgDepartment webdept = new WebV3xOrgDepartment();
		V3xOrgDepartment dept = new V3xOrgDepartment();
		//获得最大排序号
		Integer maxSortNum = orgManagerDirect.getMaxExternalDeptSortNum(CurrentUser.get().getLoginAccount());
		dept.setSortId(maxSortNum+1);
		dept.setEnabled(true);
	...
	public ModelAndView createExternalDept(HttpServletRequest request,
        ...
        dept.setIsInternal(false); 		
		V3xOrgDepartment addedDept = orgManagerDirect.addDepartment(dept,_parent_Id);
		
		// 触发创建部门事件
		eventListener.addDepartment(addedDept);
		//记录日志
		appLogManager.insertLog(user, AppLogAction.Organization_NewExternalDept, user.getName(),addedDept.getName());

		super.rendJavaScript(response, "parent.doEndExternalDept('"+accountid+"', 'add', '" + dept.getId() + "', '" + _parent_Id + "', '" + Strings.escapeJavascript(dept.getName()) + "', "+dept.getSortId()+")");
		return null;
	}
	...
	public ModelAndView editExternalDept(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/editExternalDept");
		String strid = request.getParameter("id");
		Long _id = Long.parseLong(strid);
		V3xOrgDepartment dept = orgManagerDirect.getDepartmentById(_id);
		WebV3xOrgDepartment webdept = new WebV3xOrgDepartment();
	...
	public ModelAndView updateExternalDept(HttpServletRequest request,
		...
		if(orgEnable&&(!nowEnable)){
			...
			List<V3xOrgMember> deptMembers = orgManagerDirect.getMembersByDepartment(dept.getId(), true, null,CurrentUser.get().getLoginAccount());
        ...
        String parentId = request.getParameter("parentId");	        
		//更新父部门
		V3xOrgDepartment orgParent = orgManagerDirect.getParentDepartment(new Long(deptId));	
		if(orgParent==null){
			Long pId = Long.parseLong(parentId);
			if(orgManagerDirect.getDepartmentById(pId)!=null){
				orgManagerDirect.setDepPath(dept, pId);
		...
		//记录日志
		User user = CurrentUser.get();
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateExternalDept, user.getName(),dept.getName());
	...
	public ModelAndView destroyExternalDept(HttpServletRequest request,
		...
		if (null != strids && !strids.equals("")) {
			...
			if (null != arrids && arrids.length > 0) {
				for (String id : arrids) {
					...
					deptList.add(dept1);
					//检查部门下是否存在成员
					List<V3xOrgMember> members = orgManagerDirect.getMembersByProperty("orgDepartmentId", dept1.getId(), dept1.getOrgAccountId());
					boolean isAllMemberUnEnabled = true;						
					for(V3xOrgMember mem:members){
						if(mem.getEnabled()){
							isAllMemberUnEnabled = false;
					...
					if(ListUtils.EMPTY_LIST.equals(members)||isAllMemberUnEnabled){
						...
						PrintWriter out = response.getWriter();
						out.println("<script>");
						out.println("alert('"+ Constant.getString("ext.depatition.delete.member",local)+"');");
						out.println("</script>");
						out.flush();
        ...
        //记录日志
		appLogManager.insertLogs(user, AppLogAction.Organization_DeleteExternalDept, appLogs);
	...
	public ModelAndView listExternalMember(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/listExternalMember");
		List<WebV3xOrgMember> resultlist = new ArrayList<WebV3xOrgMember>();
		long deptId = -1;
		String condition = request.getParameter("condition");
		String textfield = request.getParameter("textfield");
		List<V3xOrgMember> memberlist = new ArrayList<V3xOrgMember>();
		memberlist = OrganizationHelper.searchMember(condition, textfield, searchManager, orgManagerDirect, true, false,true);
		if (null != memberlist) {
			for (V3xOrgEntity memberEnt : memberlist) {
				...
				WebV3xOrgMember webMember = new WebV3xOrgMember();
				webMember.setV3xOrgMember(member);
				V3xOrgEntity dept = orgManagerDirect.getEntityNoRelation(V3xOrgDepartment.class.getSimpleName(), "id", deptId, CurrentUser.get().getLoginAccount());
				if (dept != null) {
					webMember.setDepartmentName(dept.getName());
				}
				//设置工作范围
		        String workscopeNames = null;
		        List<V3xOrgRelationship> rels = orgManagerDirect.getRelationships("type",V3xOrgEntity.ORGREL_TYPE_EXTERNAL_SCOPE,"sourceId",member.getId());
		        List<V3xOrgEntity> ents = new ArrayList<V3xOrgEntity>();
		        for(V3xOrgRelationship rel : rels){
		        	if(rel.getExtend3()!=null){
		            	V3xOrgEntity ent = orgManagerDirect.getEntityNoRelation(OrgHelper.getEntityType(rel.getExtend3()).getSimpleName(), "id", rel.getObjectiveId(), V3xOrgEntity.VIRTUAL_ACCOUNT_ID);
		            	if(ent!=null){
		        ...
		        if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
                {
                    ...
                    try
                    {
                        webMember.setTypeName(event.getLdapAdLoginName(member.getLoginName()));
                        modelAndView.addObject("hasLDAPAD", true);
                    }
                    catch (Exception e)
                    {
	...
	public ModelAndView addExternalMember(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/addExternalMember");
		//获得当前排序号的最大值
		Integer maxSortNum = orgManagerDirect.getMaxExternalMemberSortNum(CurrentUser.get().getLoginAccount());
		WebV3xOrgMember webMember = new WebV3xOrgMember();
		V3xOrgMember member = new V3xOrgMember();
		member.setEnabled(true);
		member.setPassword("");
	    ...
	    //List<SpaceModel> spaceList = spaceManager.getAdminCanManagerSpace(CurrentUser.get().getLoginAccount(), SpaceTypeClass.personal, "state", String.valueOf(SpaceState.normal.ordinal()));
	    //modelAndView.addObject("spaceList", spaceList);
	    //Long currentSpaceId = spaceManager.getPersonalSpaceId4Create(false,CurrentUser.get().getLoginAccount());
        //currentSpaceId
	...
	public ModelAndView createExternalMember(HttpServletRequest request,
		...
		try {
            ...
            boolean isGovVersion = (Boolean)SysFlag.is_gov_only.getFlag();  //政务版标识
            if(isGovVersion){
		    	member.setProperty("dutyLevelName", request.getParameter("dutyLevelName"));
            }
            ...
            if (null != workscopeidtype && workscopeidtype.length() > 0) {
                ...
                orgManagerDirect.updateExternalMemberWorkScope(member.getId(), rels);
            }
     		// 触发创建人员事件
			eventListener.addMember(newMember);
			//日志
			appLogManager.insertLog(user, AppLogAction.Organization_NewExternalMember, user.getName(),member.getName());
...
//	                   throw new BusinessException("ldap/ad 添加人员绑定不成功！",e);
	...
	public ModelAndView editExternalMember(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/external/editExternalMember");
		String id = request.getParameter("id");
		V3xOrgMember member = orgManagerDirect.getMemberById(Long.parseLong(id));
		long deptId = member.getOrgDepartmentId();
		WebV3xOrgMember webMember = new WebV3xOrgMember();
        ...
        if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
        {
            ...
            try
            {
            	modelAndView.addObject("ldapADLoginName", event.getLdapAdExUnitCode(member.getLoginName()));
            	modelAndView.addObject("hasLDAPAD", true);
...
//		orgManagerDirect.updateEntity(member);
		...
		if(password!=null
			...
			if(user.isAdministrator()){
				operationlogManager.insertOplog(member.getId(),ApplicationCategoryEnum.modifyPassword,
						"org.member_form.modify.password","org.member_form.modify.password.label",member.getName());					
			}else{
				operationlogManager.insertOplog(member.getId(),ApplicationCategoryEnum.modifyPassword,
						"org.member_form.modify.password","org.member_form.modify.password.hr.label",
		...
		eventListener.updateMember(memberBeforeUpdate, newMember);		
		//日志
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateExternalMember, user.getName(),member.getName());
		...
		if(LDAPConfig.getInstance().getIsEnableLdap()&&SystemEnvironment.hasPlugin(com.seeyon.v3x.product.ProductInfo.PluginNoMapper.LDAP_AD.name()))
        {
            try
            {
                ...
                if(isLoginNameModifyed)
             {
              ...
              if(event.getLdapAdExUnitCode(model.getLoginName()).equals(ldapEntry)&&!isLoginNameModifyed&&password!=null
  					&&!StringUtils.isEmpty(password)
					&&!password.equals(V3xOrgEntity.DEFAULT_INTERNAL_PASSWORD)&&!com.seeyon.v3x.common.ldap.config.LDAPConfig.getInstance().isDisabledModifyPassWord())
              {
            	  event.changePassword(memberBeforeUpdate, memberLdap);
            	  appLogManager.insertLog(user, AppLogAction.LDAP_PassWord_Update, memberLdap.getName(),ldapEntry);
              }else if(!event.getLdapAdExUnitCode(memberLdap.getLoginName()).equals(ldapEntry))
	...
	public ModelAndView destroyExternalMember(HttpServletRequest request,
		...
		for (String string : membetIds) {
			...
			if(signetManager.hasSignet(ids))
			{
				...
				out = response.getWriter();
				out.println("<script>");				
				out.println("alert(parent.v3x.getMessage('organizationLang.orgainzation_delete_member_has_signet','"+mem.getName()+"'));");
				out.println("</script>");	
				out.flush();
				return super.refreshWindow("parent");			
			}
		...
		appLogManager.insertLogs(user, AppLogAction.Organization_DeleteExternalMember, appLogs);
	...
	public ModelAndView exportExternalMember(HttpServletRequest request,
		...
		DataRecord dataRecord=null;
		try{
			dataRecord = OrganizationHelper.exportMember(request,orgManagerDirect, searchManager, condition, textfield,false);
		}catch(Exception e){
			DataUtil.removeImpExpAction(u.getId());
			throw e;
		}
	...
	public ModelAndView transfer2Internal(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView modelAndView = new ModelAndView("organization/member/editMember");
		String id = request.getParameter("id");
		V3xOrgMember member = new V3xOrgMember(orgManagerDirect.getMemberById(Long.parseLong(id)));
		int sortNum = orgManagerDirect.getMaxMemberSortNum(member.getOrgAccountId());
	...
	public ModelAndView exportDepartmentMember(HttpServletRequest request,
		...
		DataRecord dataRecord=null;
		try{
			dataRecord = OrganizationHelper.exportDepartmentMember(request, metadataManager, 
					response, fileToExcelManager, orgManagerDirect,principalManager,searchManager, pathvalue);
		}catch(Exception e){
			DataUtil.removeImpExpAction(u.getId());
			throw e;
		}
	...
	/**
	 * 单位是否允许删除判断工具类</br>
	 * 原则：单位下不能存在人员、部门、岗位、组、子单位等组织模型</br>
	...
	private boolean isAccountCanBeDeleted(Long accountId) throws Exception {
		...
		List accounts = orgManagerDirect.getChildAccount(accountId, orgManagerDirect.getAllAccounts());
	...
	private boolean checkAccountCanDel(List<V3xOrgEntity> entits) {
		...
		for(V3xOrgEntity entity : entits) {
			if(!entity.getIsDeleted()) {
				log.info("删除单位失败，该单位下存在该实体未被删除，该实体为:"+entity.getEntityType()+entity.getName());
				return false;
			}
		}
		return false;
	...
	/**
	 * 单位是否允许停用</br>
	 * 原则：不允许存在未删除或启用的人员、部门、岗位、组、子单位</br>
	 * 可以存在停用的组织模型
	 */
	...
	public ModelAndView moveDept(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/department/groupDept");
		return result;
	}
	...
	public ModelAndView moveDeptResult(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/department/groupDeptResult");
		// 获得调入单位
		...
		if (null != deptIds && !deptIds.equals("")) {			
			...
			for (String strid : deptIdArray) {				
				...
				String str1="";
				if(id!=null){
					V3xOrgDepartment dept = orgManagerDirect.getDepartmentById(id);
					str1=dept.getName();
					...
					if(validateList.size()>0){
						...
						moveLogList = organizationServices.moveDept(id, accountId);
						//重新设置部门空间的所属单位
						...
						String OrganizationResources = "com.seeyon.v3x.organization.resources.i18n.OrganizationResources";
			        	String groupName = ResourceBundleUtil.getString(OrganizationResources, "org.account_form.groupAdminName.value" + (String)SysFlag.EditionSuffix.getFlag());
						appLogManager.insertLog(CurrentUser.get(),  AppLogAction.Organization_MoveDept,  dept.getName(),  account.getName(), groupName);
						isMoveDept = true;
						...
						MoveDepartmentEvent gkeMoveDeptEvent=new MoveDepartmentEvent(this);
						gkeMoveDeptEvent.setDept(d);
						EventDispatcher.fireEvent(gkeMoveDeptEvent);
					}
					V3xOrgAccount account = orgManagerDirect.getAccountById(dept.getOrgAccountId());
					if(account!=null)
	...
	private List<String> validateMoveDept(Long deptId, Long accountId) throws Exception{
		...
		if(account != null){
			if(dept!=null){
				...
				//校验调入单位是否已经存在同级同名部门
				List<V3xOrgDepartment> accdepts = orgManagerDirect.getChildDepartments(accountId, true);		
				...
				//校验部门下人员
				List<V3xOrgEntity> deptMems = orgManagerDirect.getEntityList(V3xOrgMember.class.getSimpleName(), "orgDepartmentId", deptId, dept.getOrgAccountId());
				if(deptMems!=null){
					for(V3xOrgEntity deptMemEnt : deptMems){
                        //部门下人员的职务级别是否存在
						V3xOrgMember deptMem = (V3xOrgMember)deptMemEnt;
						V3xOrgLevel memlevel = orgManagerDirect.getLevelById(deptMem.getOrgLevelId());
						if(memlevel!=null){
							List<V3xOrgEntity> levelList = orgManagerDirect.getEntityListNoRelation(V3xOrgLevel.class.getSimpleName(), "name", memlevel.getName(), accountId);
							if(levelList==null||levelList.size() == 0){
								validateList.add("2-"+deptMem.getName()+"-"+memlevel.getName());
							}else{
								for(V3xOrgEntity level:levelList){
									if(!level.getEnabled()){
										validateList.add("3-"+deptMem.getName()+"-"+memlevel.getName());
									}
								}
							}
						}
	...
	private List<String> validateMoveMember(Long memberId, Long deptId) throws Exception{
		V3xOrgDepartment dept = orgManagerDirect.getDepartmentById(deptId);
		...
		if(dept != null){
			if(member!=null){
                //部门下人员的职务级别是否存在
				V3xOrgLevel memlevel = orgManagerDirect.getLevelById(member.getOrgLevelId());
				if(memlevel!=null){
					List<V3xOrgEntity> levelList = orgManagerDirect.getEntityListNoRelation(V3xOrgLevel.class.getSimpleName(), "name", memlevel.getName(), dept.getOrgAccountId());
					if(levelList==null||levelList.size() == 0){
						validateList.add("2-"+member.getName()+"-"+memlevel.getName());
					}else{
						for(V3xOrgEntity level:levelList){
							if(!level.getEnabled()){
								validateList.add("3-"+member.getName()+"-"+memlevel.getName());
							}
						}
					}
				}	
	...
	public ModelAndView listDutyLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		User user = CurrentUser.get();
		ModelAndView result = new ModelAndView("organization/dutyLevel/listDutyLevel");

		String condition = request.getParameter("condition");
		List<V3xOrgDutyLevel> levellist = null; 
		if(condition!=null && !"".equals(condition)){
			String value = request.getParameter("textfield"); 
			levellist = orgManager.getAllDutyLevels(user.getLoginAccount(),condition,value);
		}else{
			levellist = orgManagerDirect.getAllDutyLevels(user.getLoginAccount());
		} 
...
//		}
	...
	public ModelAndView addDutyLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/dutyLevel/addDutyLevel");
...
//		}
	...
	public ModelAndView createDutyLevel(HttpServletRequest request,
		...
		PrintWriter out = response.getWriter();
		Locale local = LocaleContext.getLocale(request);
		boolean continues = request.getParameterValues("cont") != null;  //继续添加 标识
...
//		}else{
        ...
        //记录日志
		appLogManager.insertLog(user, AppLogAction.Organization_NewDutyLevel, user.getName(),level.getName());
		return null;
		//}		
	}
	...
	public ModelAndView editDutyLevel(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView result = new ModelAndView("organization/dutyLevel/editDutyLevel");
		String id = request.getParameter("id");
		V3xOrgDutyLevel level = orgManagerDirect.getDutyLevelById(Long.parseLong(id));
		result.addObject("level", level);
//		//取得集团职务级别
//		List<V3xOrgLevel> groupLevellist = orgManagerDirect.getAllLevels(orgManagerDirect.getRootAccount().getId(),false);
//		//过滤无效的级别
...
//		}
	...
	public ModelAndView updateDutyLevel(HttpServletRequest request,
		...
		bind(request, model);
		V3xOrgDutyLevel level = orgManagerDirect.getDutyLevelById(model.getId());	
		// 为事件调用记录修改前的职务级别
...
//			String str="";
//			if(errorLevel.getLevelId().intValue()>level.getLevelId().intValue()){
//				str="alert('"+ Constant.getString("level.map.group.low",local,errorLevel.getName())+"');";
//				
//			}else{
//				str="alert('"+ Constant.getString("level.map.group.up",local,errorLevel.getName())+"');";
//				
//			}
//			super.rendJavaScript(response, "parent.toEditMember();"+str);
//			return null;
//		}else{
        ...
        //记录日志
		User user = CurrentUser.get();
		appLogManager.insertLog(user, AppLogAction.Organization_UpdateDutyLevel, user.getName(),level.getName());			
		// 提示用户操作成功
		super.rendJavaScript(response, "parent.doEndDutyLevel()");
		return null;
		//}
	...
	public ModelAndView destroyDutyLevel(HttpServletRequest request,
		...
		if (null != ids && !ids.equals("")) {
			...
			if (null != arrID && arrID.length > 0) {
				...
				for (String strID : arrID) {
					Long id = Long.parseLong(strID);
					V3xOrgDutyLevel level = orgManagerDirect.getDutyLevelById(id);
					levelList.add(level);
					
					//判断职级下是否有成员存在
					List<V3xOrgEntity> members = orgManagerDirect.getEntityList(V3xOrgMember.class.getSimpleName(), "orgDutyLevelId", id, level.getOrgAccountId());
					boolean isAllMemberUnEnabled = true;					
					for(V3xOrgEntity mem:members){
					...
					if(ListUtils.EMPTY_LIST.equals(members)||isAllMemberUnEnabled){
						...
						out.println("</script>");
						out.flush();
						return super.redirectModelAndView("/organization.do?method=organizationFrame&from=DutyLevel", "parent");
					}				
				}
                ...
                //记录日志
				appLogManager.insertLogs(user, AppLogAction.Organization_DeleteDutyLevel, appLogs);
			}
		}
		// 提示用户操作成功
		PrintWriter out = response.getWriter();
	...
	/**
	 * 停用某单位，查找该单位下启用的组织模型信息</br>
	 * 是否存在有效组织结构，部门，岗位，组，子部门，人员</br>
	 * @param list 从内存中获取该单位下的组织模型列表
	...
	private boolean checkValid4Disenable(List<V3xOrgEntity> list) {
		...
		for(V3xOrgEntity entity : list) {
			if(entity.getEnabled()) {
				log.info("停用单位失败，该单位下存在该实体未被删除或未被停用，该实体为:"+entity.getEntityType()+entity.getName());
				return true;
			}
		}
		return false;
...
//				.append((dept.getId()+"").replace("-","_"))
				...
				.append("\"")
				.append(" action=\"")
				.append("javascript:showAccountInfo('").append(dept.getId()).append("');")
				.append("\"")		
				.append(" DBlClick=\"")
				.append("javascript:editAccountInfo('").append(dept.getId()).append("');")
				.append("\"")				
				.append(" icon=\"")
